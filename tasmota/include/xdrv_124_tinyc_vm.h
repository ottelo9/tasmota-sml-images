...
// ⚠️ NO `#ifdef USE_MQTT` any more. It was never a switch anybody sets:
// Tasmota dropped it in 2017 (changelog 3.1.13, "Add option to disable MQTT as
// define USE_MQTT" — removed again once MQTT became core). `xdrv_02_9_mqtt.ino`
// does not know it, `MqttPublishPayload` is always compiled. So our gates did
// NOT "handle MQTT being off", they cut the MQTT syscalls out of EVERY build —
// mqttPublish, mqttSubscribe, OnMqttData, all of them. Found by Hans
// (2026-08-18) when mqttPublish returned -1 on his device; my own probe on the
// C3 had shown the same and I had read it as "this build has no MQTT".
...
// 1.6.6 — `udp(10, mcast_ip)` no longer uses the raw LwIP IGMP API.
// The 1.6.1-shipped raw-`igmp_leavegroup()` path was returning ERR_OK
// but multicast packets kept arriving (Andreas A/B test 15.05.) because
// Arduino-ESP32 `NetworkUDP::beginMulticast()` joins via socket-layer
// `setsockopt(IP_ADD_MEMBERSHIP)` — that writes BOTH the netif IGMP table
// AND a per-socket membership list, and the raw `igmp_leavegroup` only
// cleared the first. Replacement is `NetworkUDP::stop()` (which calls
// `setsockopt(IP_DROP_MEMBERSHIP)` on the underlying fd, atomically
// clearing both layers) followed by re-`begin(port)` for the unicast
// listener — see case 10 below. `<lwip/igmp.h>` no longer needed.
...
#endif
#include "driver/rmt_tx.h"        // DMX512 TX via RMT (no UART consumed)
...
// Outgoing TCP client slots (selected via tcpSelect). Bumped 4 → 8 in v1.5.1
// after a Modbus-TCP user (5+ Wechselrichter / BMU / Smart-Meter modules
// in parallel) ran out at 4. Each slot costs ~80 B in the Tinyc struct
// (WiFiClient object); 8 slots ≈ 640 B — fine even on ESP8266.
// Override at build time via -DTC_TCP_CLI_SLOTS=N.
#ifndef TC_TCP_CLI_SLOTS
#define TC_TCP_CLI_SLOTS     8
#endif
...
#endif
// ...and settable at runtime: `TinyCStack <bytes>` writes the value to
// /tinyc_stack.cfg and it applies from the next start of a slot. On a 4 MB C3
// with three slots, 12 KB per slot is a lot of memory a simple script never
// needs -- how much it REALLY needs is reported by the same facility as
// StackFreeMin (gemu 2026-08-17).
// Mind the lower bound: the path saveVars() -> fwrite -> LittleFS -> SPI flash
// eats about 3 KB on its own (see the crash story above).
...
// stabilize before main() runs in its own FreeRTOS task and starts
// touching peripheral hardware. Without this gate, main() runs at
// uptime ~250 ms (right after first FUNC_LOOP), still within the
// Wi-Fi-up window — and `serialBegin` claims the UART but receives
// no bytes. 3 s covers a typical Wi-Fi-up cycle. Set higher
// (`-DTC_AUTOEXEC_MIN_UPTIME=10`) for slow networks. Manual
// `TinyCRun` mid-life is unaffected — that path doesn't go through
// the autoexec spawner.
#ifndef TC_AUTOEXEC_MIN_UPTIME
...
// own RESPONSE_MAX_SIZE is ~700.
#ifndef TC_RESPONSE_MAX
#ifdef ESP8266
  #define TC_RESPONSE_MAX       256
#else  // ESP32
  #define TC_RESPONSE_MAX       512
#endif
#endif
...
enum TcOp {
  ...
  OP_JNZ          = 0x52, OP_CALL         = 0x53, OP_CALL_INDIRECT = 0x56,
  ...
  // ── Packed byte arrays (`byte[]`, one byte per element) ──────────────────
  // Every other array element in TinyC is an int32 — `char[]` included. A 1 KB
  // buffer therefore costs 4 KB, and as a LOCAL it would want 1024 of the 256
  // slots a frame has. `byte[]` lives PACKED in the same storage: n bytes take
  // (n+3)/4 slots, element i is byte i of the region. The compiler knows the
  // type at the access site and emits these opcodes directly, so a byte index
  // costs the same as an int32 index with no branch in the hot path.
  // See tinyc/docs/BYTE_ARRAYS.md.
  OP_LOAD_LOCAL_BYTE  = 0xA8,  // u8 base_idx; pop idx -> push ((uint8_t*)&locals[base])[idx]
  OP_STORE_LOCAL_BYTE = 0xA9,  // u8 base_idx; pop val, pop idx -> store byte
  OP_LOAD_GLOBAL_BYTE = 0xAA,  // u16 base_idx; pop idx -> push byte
  OP_STORE_GLOBAL_BYTE= 0xAB,  // u16 base_idx; pop val, pop idx -> store byte
  OP_LOAD_HEAP_BYTE   = 0xAC,  // u8 handle; pop idx -> push byte
  OP_STORE_HEAP_BYTE  = 0xAD,  // u8 handle; pop val, pop idx -> store byte
  OP_LOAD_REF_BYTE    = 0xAE,  // u8 local_idx; pop idx -> push byte via resolved ref
  OP_STORE_REF_BYTE   = 0xAF,  // u8 local_idx; pop val, pop idx -> store byte via ref
  ...
  OP_LK_OP_ST       = 0xB1,  // (src u8, k i8, op u8, dst u8) locals[dst] = locals[src] OP k
                             // Replaces LOAD_LOCAL·PUSH_I8·<op>·STORE_LOCAL.
  OP_LL_OP_ST       = 0xB2,  // (a u8, b u8, op u8, dst u8)   locals[dst] = locals[a] OP locals[b]
                             // Replaces LOAD_LOCAL·LOAD_LOCAL·<op>·STORE_LOCAL.
                             // `op` is the ORDINARY opcode number of the operation (OP_ADD, OP_MUL, ...),
                             // so one superinstruction covers every operator instead of one per operator,
                             // and the compiler simply passes along the opcode it would otherwise have emitted.
  OP_LK32_OP_ST     = 0xB3,  // (src u8, k i32, op u8, dst u8) locals[dst] = locals[src] OP k
                             // Wide-constant twin of 0xB1. Three bytes bigger; code size is not
                             // the bottleneck, and it covers every constant an i8 cannot hold.
  OP_LL_CMP_JZ      = 0xB4,  // (a u8, b u8, cmp u8, target u16)  if !(locals[a] CMP locals[b]) goto target
  OP_LK32_CMP_JZ    = 0xB5,  // (a u8, k i32, cmp u8, target u16) if !(locals[a] CMP k) goto target
                             // The loop head. Runs once per iteration of EVERY loop, so this is the
                             // broadest of the fusions even though it saves the same three opcodes
                             // as the others. Branch-on-FALSE, matching the JZ it replaces.
  // ⚠️ `x = y OP_OUT (z OP_IN k)` -- the nested shape, ONE opcode.
  // Replaces LOAD_LOCAL·LOAD_LOCAL·PUSH_I8·<op>·<op>·STORE_LOCAL: six opcodes in
  // 10 bytes by one in 7. After the six fusions of 2026-08-08 the bench_int loop
  // still spent 13 opcodes per iteration, and SIX of them were this one line
  // (`sum = sum - (i / 2)`). It is eight now.
  // ⚠️ The shape was COUNTED, not guessed: across the 224 shipped examples it
  // occurs ten times, every time with an i8 constant on the inside -- never with
  // three locals, never with a wide constant (2026-09-03). Hence exactly one.
  OP_LLK_OP2_ST     = 0xC2,  // (y u8, z u8, k i8, op_in u8, op_out u8, dst u8)
                             // locals[dst] = locals[y] OP_OUT (locals[z] OP_IN k)
  OP_INC_LOCAL      = 0xB0,  // (idx u8, delta i8) locals[idx] += delta, no stack effect.
                             ...
                             // is then thrown away.
  // ── Packed 16-bit arrays (`int16` / `uint16`) ────────────────────────────
  // Same idea as the byte block at 0xA8, one step coarser: an element is two
  // bytes of the region, so `n` elements take (n+1)/2 int32 slots and element
  // `i` sits at ((int16_t*)basis)[i]. Half the RAM of an int[] at the FULL
  // resolution of a sensor reading — where byte[] needs WebChartQ's scale and
  // offset to squeeze a temperature into eight bits, an int16 holds it in
  // hundredths of a kelvin outright. See tinyc/docs/INT16_ARRAYS.md.
  //
  // Signedness is a property of the LOAD, not of the storage: the stores mask
  // to 16 bits and are shared, while the compiler picks the sign- or
  // zero-extending load from the declared type at the access site. That is why
  // `uint16` costs four opcodes here instead of eight, and why the element-kind
  // field in a ref needs no third value for it.
  OP_LOAD_LOCAL_I16   = 0xB6,  // u8 base_idx; pop idx -> push (int16_t)((int16_t*)&locals[base])[idx]
  OP_STORE_LOCAL_I16  = 0xB7,  // u8 base_idx; pop val, pop idx -> store 16 bits
  OP_LOAD_GLOBAL_I16  = 0xB8,  // u16 base_idx; pop idx -> push sign-extended
  OP_STORE_GLOBAL_I16 = 0xB9,  // u16 base_idx; pop val, pop idx -> store 16 bits
  OP_LOAD_HEAP_I16    = 0xBA,  // u8 handle; pop idx -> push sign-extended
  OP_STORE_HEAP_I16   = 0xBB,  // u8 handle; pop val, pop idx -> store 16 bits
  OP_LOAD_REF_I16     = 0xBC,  // u8 local_idx; pop idx -> push sign-extended via resolved ref
  OP_STORE_REF_I16    = 0xBD,  // u8 local_idx; pop val, pop idx -> store 16 bits via ref
  // Zero-extending twins — `uint16` (Modbus holding registers, raw ADC words).
  // No stores: a store writes the same sixteen bits either way.
  OP_LOAD_LOCAL_U16   = 0xBE,  // u8 base_idx; pop idx -> push 0..65535
  OP_LOAD_GLOBAL_U16  = 0xBF,  // u16 base_idx
  OP_LOAD_HEAP_U16    = 0xC0,  // u8 handle
  OP_LOAD_REF_U16     = 0xC1,  // u8 local_idx
...
enum TcSyscall {
  ...
  SYS_FILE_READ_ARR = 82, // (arr_ref, handle) -> element_count  (FLOAT csv, Scripter fra())
  SYS_FILE_WRITE_ARR= 83, // (arr_ref, handle, append) -> void   (FLOAT csv, Scripter fwa())
  ...
  SYS_JSON_NUM        = 422, // (src_ref, path_ref) -> float — parse JSON in src buffer by '#'-path
  SYS_JSON_STR        = 423, // (src_ref, path_ref, dst_ref) -> int — JSON string value into dst, len (-1 absent)
  SYS_VAR_IDX         = 424, // (var_ref) -> int — global index of a var (for hand-built tcbtn/seva HTML)
  SYS_WEB_BUTTON_V    = 425, // (var_ref, label_ref) -> void — webButton with a runtime char[] label
  SYS_WEB_SLIDER_V    = 426, // (var_ref, min, max, label_ref) -> void — webSlider with a runtime char[] label
  SYS_FAST_MUX        = 427, // (flag, time, buf_ref, len) -> int — HW-timer GPIO multiplexer (classic ESP32 / ESP32-S3, USE_TINYC_FAST_MUX); -1 if not built
  // HTTP
  ...
  // ── String operations 1.5.0 (in-place, literal-needle variants) ─────────
  SYS_STR_REPLACE_CONST   = 302, // (arr, old_const_idx, new_const_idx) -> int — replace all occurrences in-place; returns # replacements
  SYS_STR_STARTS_CONST    = 303, // (arr, prefix_const_idx)             -> int 1/0 — does arr start with the literal prefix?
  SYS_STR_ENDS_CONST      = 304, // (arr, suffix_const_idx)             -> int 1/0 — does arr end with the literal suffix?
  SYS_STR_CONTAINS_CONST  = 305, // (arr, substr_const_idx)             -> int 1/0 — does arr contain the literal substring?
  SYS_STR_TO_UPPER        = 306, // (arr)                               -> void   — in-place upper-case conversion (ASCII A-Z only)
  SYS_STR_TO_LOWER        = 307, // (arr)                               -> void   — in-place lower-case conversion (ASCII a-z only)
  SYS_STR_TRIM            = 308, // (arr)                               -> int    — in-place strip leading + trailing whitespace; returns new length
  ...
  // slot (set via tcpSelect()). Useful for Modbus-TCP, MQTT-TLS, REST clients
  // talking to peers that idle-time out (SMA Tripower, Solar Edge, etc.) or
  // where Nagle's algorithm slows down small request/response pairs.
  SYS_TCP_KEEPALIVE        = 348, // (idle_sec, intvl_sec, count) -> int 1=ok 0=err
                                  //   Sets SO_KEEPALIVE + TCP_KEEPIDLE/INTVL/CNT
                                  //   on the selected slot. Counts as "user-set"
  ...
  SYS_TCP_NODELAY          = 349, // (on)                         -> void
                                  ...
                                  //   re-enable Nagle, on=0).
  SYS_TCP_DISCONNECT_REASON = 350, // ()                          -> int 0..5
                                  //   Last-disconnect reason for the selected
                                  //   slot. 0=never used, 1=connected,
                                  //   2=peer closed (RST/FIN), 3=local timeout,
                                  //   4=network error, 5=user closed.

  // Atomic write-then-wait-for-response on the selected TCP slot. Folds
  // tcpWriteArray + poll-loop + tcpReadArray (the canonical Modbus-TCP
  // pattern) into one syscall, eliminating the explicit `delay(2);
  // tcpAvailable() < expected; ...` race-condition every script writes by
  // hand. The wait loop uses vTaskDelay(1) between availability checks
  // so other FreeRTOS tasks (other VM slots, spawnTask workers, Tasmota
  ...
  SYS_WEB_TOGGLE            = 394, // (gref, label_const) -> void
                                  //   Latching on/off button. Renders as a full-width button
                                  //   that is green when the bound var != 0, grey when 0; the
                                  //   label has no ON/OFF text suffix. Click flips the var
                                  //   (0<->1) via seva(). This is the stateful counterpart to
                                  //   webButton (which is now a momentary action button).

  SYS_PERSIST_DUMP          = 395, // () -> int  number of persist entries
                                  //   Diagnostic, mirrors shareDump(). Logs every persist
                                  //   entry (index, slot count, raw int32 words chunked
                                  //   16/line so nothing is silently dropped) plus a header
                                  //   with the .pvs filename + layout hash. Lets a user
                                  //   snapshot all persist values to the console/MQTT log
                                  //   BEFORE a persist-layout-change flash (which resets
                                  //   ALL persist values to defaults). One-shot use.

  SYS_DMX_WRITE             = 396, // (channel, value) -> void  set DMX512 slot
                                  //   (1..TC_DMX_SLOTS) to 0..255. TX-only via the
                                  //   inverse-line BREAK method; frame auto-refreshed
                                  //   ~20 Hz, 30 s watchdog → all-zero.
  SYS_DMX_INIT              = 397, // (gpio) -> int  1=ok 0=fail
                                  //   Runtime-configure DMX512 TX on an RMT channel at
  ...
  SYS_MTR_NAME              = 408, // (ep, name) -> void  matterName(): name an endpoint (bridge: shows as the accessory title in Apple Home)
  ...
  // mqttPublish with a runtime topic/payload AND a log level.
  //   mqttPublish("topic", "payload")          -> SYS_MQTT_PUBLISH_TO  (literals, unchanged)
  //   mqttPublish(topic_buf, pay_buf [, lvl])  -> SYS_MQTT_PUBLISH_REF
  // Two things the const-only form cannot do (Hans, 2026-08-18): a topic built
  // at runtime -- his carries the device name, so he had to go through the
  // `Publish` COMMAND, parsing a command line to reach a function two frames
  // down -- and a quiet publish. A regulator that publishes every 5 s fills the
  // console at the default weblog 2, and turning the log down hides everything
  // else too. MqttPublishPayload() already takes a level and skips the log
  // entirely at LOG_LEVEL_NONE; only TinyC could not reach it.
  // lvl: 0 = silent, 1 = error, 2 = info (what the 2-arg form does), 3 = debug.
  SYS_MQTT_PUBLISH_REF      = 544, // (topic_ref, payload_ref, level) -> int 0=ok, -1=err
  ...
  SYS_TCP_TRANSACT          = 351, // (req_ref, req_len, resp_ref, resp_max, timeout_ms) -> int
                                  ...
                                  //                              available bytes up to resp_max
                                  //                              are read in one shot)
                                  //            -1               (timeout — no response within
                                  //                              `timeout_ms` since the request
                                  //                              was written)
                                  //            -2               (slot not connected, OR peer
                                  //                              dropped the link mid-wait —
                                  //                              tcpDisconnectReason() set to 2)
                                  //            -3               (bad arguments / nullptr ref /
                                  //                              negative length / etc.)
                                  //   Suitable for request/response protocols where the response
                                  //   fits in a single TCP segment (Modbus-TCP, ≤ ~256 B). For
                                  //   protocols where the response dribbles in chunks, use the
                                  //   lower-level tcpWriteArray + tcpAvailable + tcpReadArray.
  ...
  SYS_SPP_STATE       = 527, // ()                    -> int  0=off 1=ready 2=connecting 3=open 4=last connect FAILED
  ...
  SYS_SPP_DEINIT      = 533, // ()                    -> int  0; tear the stack down, ~85 KB back
  SYS_LVGL_CHART_UPD  = 534, // (chart_h, mode)       -> void 0=shift (LVGL default), 1=circular
  ...
  // ── Serial in BLOCKS ───────────────────────────────────────────────────
  // WHY this exists next to serialRead(): that one costs ONE SYSCALL PER BYTE.
  // Measured on a real ESP32-S3 (2026-09-13): 95 969 serial syscalls per
  // second, while an empty loop manages 333 333 -- about 10 us per byte. At
  // 230400 baud (23 kB/s) a quarter of the VM would go into fetching alone,
  // before any work. In blocks it is one call per round.
  // ⚠️ Takes an ARRAY ref, and accepts both kinds: int32 array (one byte per
  // slot) and packed byte[] -- like usbRead/tcpReadArray, so a bridge needs no
  // copying in between.
  // ⚠️ READ ONLY. For writing there is serialWriteBytes(h, buf, len), which
  ...
  SYS_FILE_READ_BIN   = 286, // (handle, arr_ref, count) -> int — read up-to count int32 elems from file as 4-byte LE binary; returns elements actually read (or -1 on bad args)
  SYS_FILE_WRITE_BIN  = 287, // (handle, arr_ref, count) -> int — write count int32 elems to file as 4-byte LE binary; returns elements actually written (or -1 on bad args). Works for both int[] and float[] since both are int32 in memory — the on-disk bit pattern is the same.
};
...
enum {
  ...
  // Out of memory: until now every failed allocation at load time reported
  // "Stack overflow" -- a wrong signpost that sends the search into the VM
  // stack while in truth the heap was short (Hans, 2026-08-18: "heap alloc
  // failed (8835 slots, 34 KB)" with "Load error: Stack overflow" below it).
...
typedef enum {
  ...
  TC_CB_BOOT_INIT,            // BootInit — fires once after Tasmota's GPIO/peripheral
                              //   setup is complete (first FUNC_LOOP). Equivalent of
                              //   Scripter's >BS section. Use this — NOT main() — for
                              //   serialBegin/i2cBegin/spiBegin/etc, otherwise the
                              //   peripheral can race with Tasmota's own driver init
                              //   and crash or silently fail to bind a pin.
...
typedef struct {
  ...
  // True while a spawnTask worker has BORROWED this (shared) VM — set for the
  // whole borrow including its delay() windows where the mutex is momentarily
  // released. Lets the UDP-global RX path skip injecting into a borrowed VM
...
struct TcSlot {
  ...
  bool     boot_init_pending;     // true after tc_vm_load — fires BootInit() on
                                  ...
                                  //   so that TinyCStop+TinyCRun re-fires it,
                                  //   which is critical for the dev workflow
                                  //   (every script reload needs hardware
                                  //   re-init via BootInit). Set in
                                  //   tc_vm_load; cleared after dispatch.
  char     filename[32];          // e.g. "/bresser.tcb"
  char     cmd_prefix[17];       // registered command prefix, e.g. "MP3" — LIVE
                                  //   matcher field; cleared by TinyCStopVM teardown.
  char     cmd_prefix_saved[17];  // STICKY copy of the registered prefix. Set alongside
                                  //   cmd_prefix when main() calls addCommand(); cleared
                                  //   ONLY on a deliberate stop / reset / program-replace
                                  //   (not on a transient worker stop+resume). Lets the
                                  //   live cmd_prefix be re-applied on restart and by the
                                  //   per-second self-heal, so a transient stop (SD->Flash
                                  //   sync, heap-tight boot) or a main() that never reaches
                                  //   addCommand() can't leave the slot Running:1 but
                                  //   BAT*/commands silently {"Command":"Unknown"}.
  // Output buffer
  ...
  volatile bool main_done;        // main() (Phase 1) has finished — set by tc_vm_task once the
                                  //   main loop exits, cleared by TinyCStartVM before the task
                                  //   spawns. The autoexec boot loader waits on this to load
                                  //   slots SERIALLY (one main() at a time) so concurrent boot
                                  //   load can't starve a heap-tight S3/PSRAM of the headroom
                                  //   main() needs to reach addCommand() (Andreas .107).
  volatile bool hold_taskloop;    // two-phase boot: when set, the VM task BLOCKS after main()
                                  //   (before Phase 2 / TaskLoop) until the autoexec loader
                                  //   clears it on ALL slots. So while slot-3's main() runs,
                                  //   slots 0..2 are parked here instead of spinning their
                                  //   TaskLoops — no running TaskLoop competes for VM time/heap
                                  //   with a still-initialising slot (the slot-3 starvation
                                  //   Andreas hit; serial start alone wasn't enough).
  volatile bool task_stop;        // signal task to stop
  volatile bool fs_was_running;   // task was running and got stopped for a flash-FS write
                                  //   (restart it when the write completes — see TinyCFsWritePause)
                                  //   (set by tc_vm_task/spawn when tc_global_pause
                                  //   is asserted; the FS-upload writer waits for
                                  //   this so its flash erase/program can't run
...
struct TINYC {
  ...
  struct {
    ...
    bool autoexec;                // auto-run on boot
    char cmd_prefix[17];          // DURABLE copy of the slot's registered command prefix
                                  //   (e.g. "BAT"). Learned from cmd_prefix_saved the first time
                                  //   main() runs addCommand(), persisted in /tinyc.cfg, and used
                                  //   by TinyCPrefixReheal to re-register a RUNNING slot's prefix
                                  //   even on a heap-tight boot where this boot's main() never
                                  //   reached addCommand() (cmd_prefix_saved is RAM/BSS, lost on
                                  //   reboot). Cleared on program-replace/reset.
...
#define TC_HTTP_MAX_HEADERS 4
  ...
  //   1 = TC_TCP_REASON_CONNECTED (currently up, no disconnect since connect)
  //   2 = TC_TCP_REASON_PEER_CLOSED (RST or FIN — LwIP doesn't easily split)
  //   3 = TC_TCP_REASON_TIMEOUT (read/write returned 0 with !connected())
  //   4 = TC_TCP_REASON_NETWORK (connect failed, no route, no network)
  //   5 = TC_TCP_REASON_USER_CLOSED (script called tcpDisconnect)
  uint8_t    tcp_cli_reason[TC_TCP_CLI_SLOTS];
...
// int32 arrays onto the stack. A topic is a path, a payload is usually a number
// or a small JSON -- longer than this gets truncated, not rejected.
#define TC_MQTT_TOPIC_MAX    128
#define TC_MQTT_PAYLOAD_MAX  512
...
// WebChartQ(scale, offset): affine decode of the NEXT WebChart's samples,
// value = raw * scale + offset. Identity by default, so an unset pair costs
// nothing and changes nothing. Deliberately ONE-SHOT -- it is cleared again by
// the WebChart that consumes it, unlike WebChartSize, whose stickiness across
// slots is the standing complaint about chart layout. One WebChart call is one
// series, so a per-series quantisation is exactly the right granularity: two
// byte[] series in one chart routinely need different scales.
static float    tc_chart_q_scale  = 1.0f;
static float    tc_chart_q_offset = 0.0f;

// Every HTTP response starts with a clean chart state. The Google Charts
// loader is emitted once per PAGE, guarded by tc_chart_lib_sent -- so if the
// flag survives into the next response, that page silently renders empty
// charts. It was cleared before /tc_ui and before WebPage(), but NOT for a
// webOn handler serving its own page: the first /chart after a boot worked,
// every later one did not, and opening /tc_ui in between made it work again.
// That intermittency is what cost the time (Hans, 2026-08-18).
static inline void tc_chart_response_reset(void) {
  tc_chart_seq       = 0;
  tc_chart_lib_sent  = false;
  tc_chart_width     = 0;
  tc_chart_height    = 0;
  tc_chart_time_base = 0;
  tc_chart_q_scale   = 1.0f;
  tc_chart_q_offset  = 0.0f;
}
...
// ── Element-kind field in a ref ─────────────────────────────────────────────
// The ref encoding is fully used in all three forms, but a pair of bits is
// free in each (see docs/BYTE_ARRAYS.md, docs/INT16_ARRAYS.md): heap bits 8-9,
// global 16-17, local 24-25. That mapping lives ONLY here so it does not
// reappear at 156 call sites.
//
//   00 = int32 (the default, and what every ref meant before byte[])
//   01 = packed byte[]      -- UNCHANGED from the single-bit scheme
//   10 = packed int16[]     (signed)
//   11 = packed uint16[]    (unsigned)
//
// Signedness is normally a property of the LOAD opcode, not of the storage --
// but a ref outlives the access site: a syscall that reads the array (WebChart)
// has only the ref to go on, and would draw a uint16 sample of 60000 as -5536.
// So the two 16-bit widths get their own code rather than sharing one.
//
// ⚠️ The `01` encoding is deliberately the old byte flag, bit-for-bit. Every
// .tcb already in the field keeps meaning exactly what it meant, so widening
// this from a flag to a field is not an ABI break by itself — only the new
// OPCODES are (see TC_SYSCALL_ABI).
#define TC_REF_BYTES_HEAP   0x00000100u
#define TC_REF_BYTES_GLOBAL 0x00010000u
#define TC_REF_BYTES_LOCAL  0x01000000u
#define TC_REF_I16_HEAP     0x00000200u
...
#define TC_ELEM_I16   2
#define TC_ELEM_U16   3

static inline uint32_t tc_ref_bytes_bit(int32_t ref) {
  uint32_t uref = (uint32_t)ref;
  switch (uref >> 30) {
    case 3: return TC_REF_BYTES_HEAP;      // heap (const refs carry bit 15)
    case 2: return TC_REF_BYTES_GLOBAL;
    default: return TC_REF_BYTES_LOCAL;    // tag 0/1 = local
  }
}

// Bit 1 of the field. Set alone it means int16; together with the byte bit
// (i.e. `11`) it means uint16.
static inline uint32_t tc_ref_i16_bit(int32_t ref) {
  uint32_t uref = (uint32_t)ref;
  switch (uref >> 30) {
    case 3: return TC_REF_I16_HEAP;
    case 2: return TC_REF_I16_GLOBAL;
    default: return TC_REF_I16_LOCAL;
  }
}

// Both bits of the field, for stripping.
static inline uint32_t tc_ref_kind_mask(int32_t ref) {
  return tc_ref_bytes_bit(ref) | tc_ref_i16_bit(ref);
}

// Which element kind does this ref carry? Const-pool refs (tag 3 with bit 15)
// are string literals: they are char data, never a packed numeric array.
static inline int tc_ref_elem_kind(int32_t ref) {
  uint32_t uref = (uint32_t)ref;
  if ((uref >> 30) == 3 && (uref & 0x8000u)) return TC_ELEM_I32;   // Konstante
  int kind = 0;
  if (uref & tc_ref_bytes_bit(ref)) kind |= 1;
  if (uref & tc_ref_i16_bit(ref))   kind |= 2;
  return kind;
}

// Bytes per element: 4, 1 or 2.
static inline int tc_ref_elem_size(int32_t ref) {
  switch (tc_ref_elem_kind(ref)) {
    case TC_ELEM_BYTE: return 1;
    case TC_ELEM_I16:
    case TC_ELEM_U16:  return 2;
    default:           return 4;
  }
}

// Does this ref point at a packed byte[]? Kept as its own predicate because 64
// syscall bodies ask exactly this question and nothing more.
static inline bool tc_ref_is_bytes(int32_t ref) {
  return tc_ref_elem_kind(ref) == TC_ELEM_BYTE;
}

// Packed 16-bit, either signedness.
static inline bool tc_ref_is_i16(int32_t ref) {
  const int k = tc_ref_elem_kind(ref);
  return k == TC_ELEM_I16 || k == TC_ELEM_U16;
}

// ...and whether that 16-bit array is the signed one.
static inline bool tc_ref_i16_signed(int32_t ref) {
  return tc_ref_elem_kind(ref) == TC_ELEM_I16;
}

// Mark a ref — the compiler does this when it builds one, the firmware when it
// passes one along.
static inline int32_t tc_ref_mark_bytes(int32_t ref) {
  return (int32_t)(((uint32_t)ref & ~tc_ref_kind_mask(ref)) | tc_ref_bytes_bit(ref));
}

static inline int32_t tc_ref_mark_i16(int32_t ref) {
  return (int32_t)(((uint32_t)ref & ~tc_ref_kind_mask(ref)) | tc_ref_i16_bit(ref));
}

static inline int32_t tc_ref_mark_u16(int32_t ref) {
  return (int32_t)(((uint32_t)ref & ~tc_ref_kind_mask(ref))
                   | tc_ref_i16_bit(ref) | tc_ref_bytes_bit(ref));
}

// Strip the field again so the existing resolution arithmetic is untouched.
static inline int32_t tc_ref_strip_kind(int32_t ref) {
  return (int32_t)((uint32_t)ref & ~tc_ref_kind_mask(ref));
}

static int32_t* tc_resolve_ref(TcVM *vm, int32_t ref) {
  // A byte[] flag would break the slot arithmetic; callers that want the int32
  // pointer get it to the START of the region (the byte view sits on top).
  ref = tc_ref_strip_kind(ref);
  uint32_t uref = (uint32_t)ref;
  uint8_t tag = uref >> 30;
  if (tag == 3) {
...
static int32_t tc_ref_maxlen_slots(TcVM *vm, int32_t ref);   // fwd, defined below

static int32_t tc_ref_maxlen(TcVM *vm, int32_t ref) {
  // For a packed byte[] the capacity is asked in BYTES, not slots — four bytes
  // per slot. The kind field has to come off first or the slot arithmetic below
  // is wrong.
  //
  // ⚠️ int16[] is deliberately NOT handled here, and that is not an oversight.
  // 121 call sites read this function under one contract — "byte[] gives bytes,
  // anything else gives slots" — and most of them go on to write `cap` int32
  // slots. Teaching it a THIRD unit would have made every one of those write
  // twice past the end of an int16 array: precisely the class of bug that ABI 26
  // exists to have fixed, multiplied by 121. So the contract stays, an int16 ref
  // reads as its slot count here (always an UNDER-estimate, therefore safe), and
  // the two places that genuinely want elements ask tc_ref_elem_count() instead.
  const bool ist_bytes = tc_ref_is_bytes(ref);
  ref = tc_ref_strip_kind(ref);
  uint32_t uref = (uint32_t)ref;
  uint8_t tag = uref >> 30;
  if (ist_bytes) {
    const int32_t slots = tc_ref_maxlen_slots(vm, ref);
    return slots > 0 ? slots * 4 : 0;
  }
  if (tag == 3) {
...
// Capacity in SLOTS — the arithmetic tc_ref_maxlen does for int32 arrays. Split
// out so the byte view can use it without recursing into itself.
static int32_t tc_ref_maxlen_slots(TcVM *vm, int32_t ref) {
  uint32_t uref = (uint32_t)tc_ref_strip_kind(ref);
  uint8_t tag = uref >> 30;
  if (tag == 3) {
    if (tc_is_const_ref(ref)) return 0;
    uint8_t handle = (uint8_t)(uref & 0xFF);
    uint16_t offset = (uint16_t)((uref >> 16) & 0x3FFF);
    if (handle < TC_MAX_HEAP_HANDLES && vm->heap_handles && vm->heap_handles[handle].alive) {
      uint16_t sz = vm->heap_handles[handle].size;
...
// Capacity in ELEMENTS, whatever the element width — the unit a syscall wants
// when it walks an array as the script declared it. Separate from
// tc_ref_maxlen() on purpose: see the warning there. Only WebChart and
// sortArray need this; everything byte- or text-facing keeps the old contract.
static int32_t tc_ref_elem_count(TcVM *vm, int32_t ref) {
  if (tc_ref_is_i16(ref)) {
    const int32_t slots = tc_ref_maxlen_slots(vm, ref);
    return slots > 0 ? slots * 2 : 0;
  }
  return tc_ref_maxlen(vm, ref);   // byte[] already answers in elements
}

// ── Byte view of an array, whichever kind it is ─────────────────────────────
// The byte-facing syscalls (file I/O, TCP/UDP, crypto, hex2bin, serial) convert
// element by element today:
//     for (int i = 0; i < n; i++) tmp[i] = (char)(arr[i] & 0xFF);
// With a packed byte[] that is pointless — the bytes already sit that way in
// memory. This helper serves both behind one signature:
//   byte[]      -> pointer DIRECTLY into the region (no copy)
//   int32 array -> `tmp` filled as before (low byte of each element)
// `want` is the requested length, clamped to the capacity. Returns NULL when
...
static uint8_t *tc_ref_bytes(TcVM *vm, int32_t ref, int32_t want,
  ...
  const bool ist_bytes = tc_ref_is_bytes(ref);
  int32_t cap = tc_ref_maxlen(vm, ref);          // byte[] -> bytes, else slots
  if (want < 0 || want > cap) want = cap;
  int32_t *basis = tc_resolve_ref(vm, ref);
  if (!basis) { if (out_len) *out_len = 0; return nullptr; }
  if (ist_bytes) {
    if (out_len) *out_len = want;
    return (uint8_t *)basis;                     // already packed
  }
  if (!tmp || tmpcap <= 0) { if (out_len) *out_len = 0; return nullptr; }
  if (want > tmpcap) want = tmpcap;
  for (int32_t i = 0; i < want; i++) tmp[i] = (uint8_t)(basis[i] & 0xFF);
  if (out_len) *out_len = want;
  return tmp;
}

// The write-back companion to tc_ref_bytes(): put `n` bytes from `src` INTO the
// array `ref`, whichever kind it is. This is the direction a byte-facing syscall
// takes for its RESULT (a digest, decrypted plaintext, bytes read off a socket).
//   byte[]      -> memcpy straight into the packed region (one byte per byte)
//   int32 array -> one byte per slot, as the syscalls wrote back by hand before
// `n` is clamped to the capacity. Returns the number of bytes written.
static int32_t tc_ref_put_bytes(TcVM *vm, int32_t ref, const uint8_t *src, int32_t n) {
  if (!src || n <= 0) return 0;
  int32_t cap = tc_ref_maxlen(vm, ref);          // byte[] -> bytes, else slots
  if (n > cap) n = cap;
  int32_t *basis = tc_resolve_ref(vm, ref);
  if (!basis) return 0;
  if (tc_ref_is_bytes(ref)) {
    // memmove, not memcpy: for an in-place op on a byte[] the result already
    // lives in the region (src points into it), so this call is a safe no-op.
    memmove((uint8_t *)basis, src, n);
  } else {
    for (int32_t i = 0; i < n; i++) basis[i] = (int32_t)src[i];
  }
  return n;
}
...
static int tc_ref_to_cstr(TcVM *vm, int32_t ref, char *out, int maxOut) {
  ...
  int32_t maxLen = tc_ref_maxlen(vm, ref);   // byte[] -> bytes, else slots
  int i;
  if (tc_ref_is_bytes(ref)) {
    // A byte[] holding text reads one byte per char, straight from the region.
...
static int tc_cstr_to_ref(TcVM *vm, int32_t ref, const char *src) {
  int32_t *buf = tc_resolve_ref(vm, ref);
  if (!buf || !src) return 0;
  int32_t maxLen = tc_ref_maxlen(vm, ref) - 1;   // byte[] -> bytes, else slots
  int i;
  if (tc_ref_is_bytes(ref)) {
    uint8_t *bp = (uint8_t *)buf;
    for (i = 0; i < maxLen && src[i]; i++) bp[i] = (uint8_t)src[i];
    bp[i] = 0;
  } else {
    for (i = 0; i < maxLen && src[i]; i++) buf[i] = (int32_t)(uint8_t)src[i];
    buf[i] = 0;
  }
  return i;
}
...
static void tc_stream_ref(TcVM *vm, int32_t ref, tc_send_fn sendFn) {
  ...
  if (!buf) return;
  int32_t maxLen = tc_ref_maxlen(vm, ref);   // byte[] -> bytes, else slots
  const bool is_bytes = tc_ref_is_bytes(ref);
  const uint8_t *bp = (const uint8_t *)buf;
  char chunk[TC_STREAM_CHUNK];
  int ci = 0;
  for (int i = 0; i < maxLen; i++) {
    uint8_t c = is_bytes ? bp[i] : (uint8_t)(buf[i] & 0xFF);
    if (c == 0) break;
    chunk[ci++] = (char)c;
...
#define TC_PWL_MAX_BINDS 24
  ...
  static String tc_pwl_get_cookie(void) {
    ...
    // No additional timeout caps — exact Scripter Powerwall.h
    // pattern. Earlier we added setHandshakeTimeout / setTimeout
    // on basic_client to bound stuck-state windows; observed on a
    // sister device that the Scripter (no caps) runs for months
    // without sticking, while our TinyC (with caps) sticks within
    // ~30 connect cycles. Hypothesis: BearSSL ECDSA handshake
    // occasionally exceeds the cap → aborted handshake leaves
    // BearSSL state half-initialized → cumulative corruption.
  ...
  static int32_t tc_pwl_get_request(TcVM *vm, const String &url) {
    ...
    // No additional timeout caps — see tc_pwl_get_cookie for rationale.
...
void tc_udp_on_receive(const char *name, char umode, const char *data, int datalen) {
  ...
  for (int si = 0; si < TC_MAX_VMS; si++) {
...
#ifdef ESP32
    // Non-blocking (drop-on-busy): if the VM is busy — e.g. a spawnTask TLS
    // worker holds the mutex for the duration of a multi-second BearSSL
    // request — DROP this global injection instead of blocking loopTask.
    ...
    // Also skip a VM a spawnTask worker has BORROWED: during the worker's
    // delay() the mutex is free and the VM looks halted, but injecting here
    // disturbs the worker's borrowed loop state -> the worker exits silently at
    // its first delay (the "no values" symptom).
    if (!s->vm.halted || s->vm.error != TC_OK || s->vm.worker_borrowed) {
#ifdef ESP32
...
// Integer binary operator, selected by the ORDINARY opcode number of the
// operation. The superinstructions (0xB1/0xB2) route through this so the fused
// and the unfused form can never drift apart: there is one definition of what
// OP_DIV does, not one per dispatch path per fusion.
// FLOATS DELIBERATELY ABSENT — the compiler's peephole refuses float operands,
// because an integer ADD on an IEEE bit pattern yields a denormal, not a+1.
// Returns TC_OK, or an error (division by zero, unknown operator) in *err.
static inline int32_t tc_binop_i(uint8_t op, int32_t a, int32_t b, int *err) {
  switch (op) {
    case OP_ADD:     return a + b;
    case OP_SUB:     return a - b;
    case OP_MUL:     return a * b;
    case OP_DIV:     if (!b) { *err = TC_ERR_DIV_ZERO; return 0; } return a / b;
    case OP_MOD:     if (!b) { *err = TC_ERR_DIV_ZERO; return 0; } return a % b;
    ...
    case OP_LTE:     return a <= b ? 1 : 0;
    case OP_GTE:     return a >= b ? 1 : 0;
    default:         *err = TC_ERR_BAD_OPCODE; return 0;   // crafted bytecode
  }
}
...
static void tc_heap_coalesce(TcVM *vm) {
  ...
  while (again) {
    ...
    for (int i = 0; i < vm->heap_handle_count; i++) {
      ...
      uint16_t end = vm->heap_handles[i].offset + vm->heap_handles[i].size;
      for (int j = 0; j < vm->heap_handle_count; j++) {
        if (j == i || vm->heap_handles[j].alive || vm->heap_handles[j].size == 0) continue;
        if (vm->heap_handles[j].offset == end) {           // j sits right after i
          vm->heap_handles[i].size += vm->heap_handles[j].size;
          vm->heap_handles[j].size = 0;                    // give up the descriptor
          end = vm->heap_handles[i].offset + vm->heap_handles[i].size;
          again = true;
        }
      }
      // If the free block is at the very top, the bump pointer walks back --
      // then the next allocation can be of any size.
...
static int tc_heap_alloc(TcVM *vm, uint16_t size) {
  ...
  // Look for an UNUSED descriptor. ⚠️ `!alive` alone is no longer enough since
  // blocks are reused: a dead descriptor with size > 0 IS A FREE BLOCK and must
  // not be overwritten as if it were an empty entry -- its space would be lost
  // forever. If none is found, the free blocks are merged first (that returns
  // entries).
  int handle = -1;
  for (int round = 0; round < 2 && handle < 0; round++) {
    for (int i = 0; i < TC_MAX_HEAP_HANDLES; i++) {
      if (!vm->heap_handles[i].alive && vm->heap_handles[i].size == 0) { handle = i; break; }
    }
    if (handle < 0 && round == 0) tc_heap_coalesce(vm);
  }
  if (handle < 0) return -1;

  // ── Serve from what we already have first ────────────────────────────────
  // The heap used to grow on EVERY allocation, even with an equally large freed
  // block right next to it: a script that creates and releases arrays (charts!)
  // drove consumption monotonically up until TC_MAX_HEAP was reached or the
  // growth failed on a fragmented system heap. Now the best-fitting free block
  // is taken -- "best fitting" so a small request does not carve up a large
  // block.
  {
    int best = -1;
    ...
    for (int i = 0; i < vm->heap_handle_count; i++) {
      ...
      if (best < 0 || vm->heap_handles[i].size < best_size) {
        best = i; best_size = vm->heap_handles[i].size;
      }
    }
    if (best >= 0) {
      // Split off the remainder when it is worth it AND a descriptor is free;
      // otherwise the request gets the whole block (the few extra slots are
      // ours, and on free they all come back).
      uint16_t rest = best_size - size;
      if (rest >= 16) {
        int spare = -1;
        for (int i = 0; i < TC_MAX_HEAP_HANDLES; i++) {
          if (i != best && !vm->heap_handles[i].alive && vm->heap_handles[i].size == 0) {
            spare = i; break;
          }
        }
        if (spare >= 0) {
          vm->heap_handles[spare].offset = vm->heap_handles[best].offset + size;
          vm->heap_handles[spare].size   = rest;
          vm->heap_handles[spare].alive  = false;
          if ((uint8_t)(spare + 1) > vm->heap_handle_count) vm->heap_handle_count = spare + 1;
          vm->heap_handles[best].size = size;
        }
      }
      vm->heap_handles[best].alive = true;
      memset(&vm->heap_data[vm->heap_handles[best].offset], 0,
             vm->heap_handles[best].size * sizeof(int32_t));
      return best;
    }
  }
...
// ---------------------------------------------------------------------------
// DMX512 TX engine (TX-only, ESP32) via the RMT peripheral — NO UART
// consumed (UARTs stay free for SML/Modbus/tc_serial). The whole frame
// (BREAK + MAB + start code + slots, each byte = 1 start-low + 8 data
// LSB-first + 2 stop-high at 4 us/bit) is built as rmt_symbol_word_t[]
// and pushed with a copy encoder at 1 MHz tick (exact, hardware-clocked
// — no CPU busy-wait BREAK). IDF5 RMT API = matches pinned arduino-esp32.
//
//   dmxInit(gpio)        -> 1 ok / 0 fail   — runtime, picks an RMT chan
//   dmxWrite(channel,val)                   — set a slot (1..TC_DMX_SLOTS)
// tc_dmx_tick() (FUNC_EVERY_50_MSECOND, ~20 Hz) re-sends the frame and
// runs a 30 s watchdog → all-zero (heater safe-off). #define TC_DMX_GPIO
// for compile-time auto-init.
// ---------------------------------------------------------------------------
#ifdef ESP32
#ifndef TC_DMX_GPIO
#define TC_DMX_GPIO       -1      // compile-time fallback (dmxInit overrides)
#endif
#ifndef TC_DMX_SLOTS
#define TC_DMX_SLOTS      16
#endif
#ifndef TC_DMX_BREAK_US
#define TC_DMX_BREAK_US   120     // >=88us spec; 120 is safely inside
#endif
#ifndef TC_DMX_MAB_US
#define TC_DMX_MAB_US     16      // >=8us spec
#endif
#ifndef TC_DMX_WATCHDOG_MS
#define TC_DMX_WATCHDOG_MS 30000  // no dmxWrite for this long -> all-zero
#endif
#define TC_DMX_BITS   (11 * (1 + TC_DMX_SLOTS))            // 11 bits/byte
#define TC_DMX_CELLS  (TC_DMX_BITS + (TC_DMX_BITS & 1))    // pad even
#define TC_DMX_NSYM   (1 + (TC_DMX_CELLS / 2))             // +1 BREAK/MAB

static rmt_channel_handle_t tc_dmx_chan = nullptr;
static rmt_encoder_handle_t tc_dmx_enc  = nullptr;
static rmt_symbol_word_t tc_dmx_sym[TC_DMX_NSYM];
static uint8_t  tc_dmx_buf[TC_DMX_SLOTS];     // slot data (channel 1 = [0])
static bool     tc_dmx_active = false;
static uint32_t tc_dmx_last_set_ms = 0;
static bool     tc_dmx_warned = false;

// Build the whole DMX frame into tc_dmx_sym[]: symbol 0 = BREAK+MAB,
// then 4 us bit-cells packed 2 per symbol (line idle = high).
static void tc_dmx_build(void) {
  tc_dmx_sym[0].duration0 = TC_DMX_BREAK_US; tc_dmx_sym[0].level0 = 0;
  tc_dmx_sym[0].duration1 = TC_DMX_MAB_US;   tc_dmx_sym[0].level1 = 1;
  uint8_t bits[TC_DMX_CELLS];
  int n = 0;
  for (int by = 0; by <= TC_DMX_SLOTS; by++) {        // [0]=start code 0x00
    uint8_t v = by ? tc_dmx_buf[by - 1] : 0;
    bits[n++] = 0;                                    // start bit (low)
    for (int b = 0; b < 8; b++) bits[n++] = (v >> b) & 1;  // LSB first
    bits[n++] = 1; bits[n++] = 1;                     // 2 stop bits (high)
  }
  while (n < TC_DMX_CELLS) bits[n++] = 1;             // pad high (idle)
  int s = 1;
  for (int k = 0; k < TC_DMX_CELLS; k += 2, s++) {
    tc_dmx_sym[s].duration0 = 4; tc_dmx_sym[s].level0 = bits[k];
    tc_dmx_sym[s].duration1 = 4; tc_dmx_sym[s].level1 = bits[k + 1];
  }
}
...
static int tc_dmx_init(int gpio) {
  ...
  if (tc_dmx_chan) {                                  // reconfigure
    rmt_disable(tc_dmx_chan);
    if (tc_dmx_enc) { rmt_del_encoder(tc_dmx_enc); tc_dmx_enc = nullptr; }
    rmt_del_channel(tc_dmx_chan); tc_dmx_chan = nullptr;
  }
  rmt_tx_channel_config_t cc = {};
  cc.gpio_num        = (gpio_num_t)gpio;
  cc.clk_src         = RMT_CLK_SRC_DEFAULT;
  cc.resolution_hz   = 1000000;                       // 1 tick = 1 us
  cc.mem_block_symbols = 64;
  cc.trans_queue_depth = 2;
  if (rmt_new_tx_channel(&cc, &tc_dmx_chan) != ESP_OK) {
    tc_dmx_chan = nullptr;
    AddLog(LOG_LEVEL_INFO, PSTR("TCC: dmxInit RMT chan alloc failed"));
    return 0;
  }
  rmt_copy_encoder_config_t ec = {};
  if (rmt_new_copy_encoder(&ec, &tc_dmx_enc) != ESP_OK ||
      rmt_enable(tc_dmx_chan) != ESP_OK) {
    if (tc_dmx_enc) { rmt_del_encoder(tc_dmx_enc); tc_dmx_enc = nullptr; }
    rmt_del_channel(tc_dmx_chan); tc_dmx_chan = nullptr;
    AddLog(LOG_LEVEL_INFO, PSTR("TCC: dmxInit RMT enable failed"));
    return 0;
  }
  memset(tc_dmx_buf, 0, sizeof(tc_dmx_buf));
  tc_dmx_build();
  tc_dmx_active = true;
  tc_dmx_last_set_ms = millis();
  AddLog(LOG_LEVEL_INFO, PSTR("TCC: DMX TX RMT GPIO%d (%d slots)"),
         gpio, TC_DMX_SLOTS);
  return 1;
}

static void tc_dmx_ensure_init(void) {
  if (tc_dmx_active) return;
  if (TC_DMX_GPIO >= 0) { tc_dmx_init(TC_DMX_GPIO); return; }
  if (!tc_dmx_warned) {
    tc_dmx_warned = true;
    AddLog(LOG_LEVEL_INFO, PSTR("TCC: dmxWrite ignored — call "
           "dmxInit(gpio) first (or #define TC_DMX_GPIO)"));
  }
}

// channel 1..TC_DMX_SLOTS, value 0..255
static void tc_dmx_set(int ch, int val) {
  tc_dmx_ensure_init();
  if (!tc_dmx_active) return;
  if (ch < 1 || ch > TC_DMX_SLOTS) return;
  if (val < 0) val = 0; if (val > 255) val = 255;
  tc_dmx_buf[ch - 1] = (uint8_t)val;
  tc_dmx_last_set_ms = millis();
}

// Called from FUNC_EVERY_50_MSECOND (~20 Hz). Rebuild + re-send + watchdog.
static void tc_dmx_tick(void) {
  if (!tc_dmx_active || !tc_dmx_chan || !tc_dmx_enc) return;
  if ((uint32_t)(millis() - tc_dmx_last_set_ms) > TC_DMX_WATCHDOG_MS) {
    memset(tc_dmx_buf, 0, sizeof(tc_dmx_buf));   // safe-off (heater 0 W)
  }
  tc_dmx_build();
  rmt_transmit_config_t tx = {};
  tx.loop_count = 0;
  tx.flags.eot_level = 1;                        // idle high after frame
  rmt_tx_wait_all_done(tc_dmx_chan, 0);          // don't queue-overrun
...
// ── Byte-aware char access for the string-write syscalls ─────────────────────
// A string element is normally one int32 slot; in a packed byte[] it is one
// byte. `ib` (= tc_ref_is_bytes(ref), computed once per buffer by the caller)
// selects which. The index is a CHARACTER position in both cases — the helper
// hides the packing, so a syscall's own loops/shifts stay index-based and work
// on either kind. char[] behaviour is byte-identical (ib=false path).
static inline int32_t tc_chr_get(const int32_t *base, bool ib, int32_t i) {
  return ib ? (int32_t)((const uint8_t *)base)[i] : base[i];
}
static inline void tc_chr_put(int32_t *base, bool ib, int32_t i, int32_t ch) {
  if (ib) ((uint8_t *)base)[i] = (uint8_t)ch;
  else    base[i] = ch;
}
// strlen over a resolved buffer, byte-aware. `cap` is the char capacity
// (= tc_ref_maxlen: bytes for byte[], slots for int32 — both are char counts).
static inline int32_t tc_str_len_b(const int32_t *base, bool ib, int32_t cap) {
  int32_t i = 0;
  while (i < cap && tc_chr_get(base, ib, i) != 0) i++;
  return i;
}
// Copy C-string `s` into `base` at char offset `ofs`, NUL-terminated, bounded
// by `cap` chars. Returns chars written (excluding NUL). Byte-aware. This is
// tc_sprintf_to_ref generalised to a byte[]-or-int32 target with an offset, so
// the sprintf _CAT variants no longer need int32 `dst + ofs` pointer maths.
static inline int32_t tc_str_write_b(int32_t *base, bool ib, int32_t cap,
                                     int32_t ofs, const char *s) {
  int32_t len = (int32_t)strlen(s);
  int32_t room = cap - 1 - ofs;
  if (room < 0) room = 0;
  if (len > room) len = room;
  for (int32_t i = 0; i < len; i++) tc_chr_put(base, ib, ofs + i, (int32_t)(uint8_t)s[i]);
  tc_chr_put(base, ib, ofs + len, 0);
  return len;
}
...
static int32_t tc_sprintf_float(char *out, int outSize, const char *fmt, float fval) {
  ...
  // Prefix and suffix copies must un-escape `%%` → `%` since this
  // function bypasses libc snprintf (the manual byte-walk above would
  // otherwise emit `%%` verbatim — visible in the WebUI as e.g. "85 %%
  // (12.4 kWh)" instead of "85 % (12.4 kWh)" on a row that mixed an
  // int specifier and a literal-`%` ahead of a float specifier).
  // Prefix scan already skips `%%` while hunting for the active spec,
  // so any `%%` we encounter here is a guaranteed escape.
  int pos = 0;
  // Copy prefix (text before %), with %% → % unescape
  ...
  // Copy suffix (text after format specifier), with %% → % unescape
...
// 1.6.23 — bounded try-lock for HTTP-side readers. Andreas's .104 (C3 single-core,
// ESP32-C3 + TinyC ebusD reader) saw the HTTP server hang ≈2:30 min after either
// /ufsu uploads or normal /cm?cmnd=WP+B polling while slot-0 was emitting 60
// shareSetFloat/s. Both lock callers held portMAX_DELAY, so any interleaving of
// the writer-burst and the HTTP-task reader could pile up enough scheduler
// pressure to wedge AsyncTCP. Readers now drop the read on a brief timeout
// (returning 0/0.0/"" — same as a missing key); writers keep portMAX_DELAY
// because they MUST persist their value (called from EverySecond at fixed rate).
// Returns true if the lock was acquired.
static inline bool tc_share_try_lock_ms(uint32_t timeout_ms) {
  if (!tc_share_mutex) tc_share_mutex = xSemaphoreCreateMutex();
  if (!tc_share_mutex) return false;
  return xSemaphoreTake(tc_share_mutex, pdMS_TO_TICKS(timeout_ms)) == pdTRUE;
}
...
#else
static inline void tc_share_lock(void)   {}
static inline bool tc_share_try_lock_ms(uint32_t /*timeout_ms*/) { return true; }
static inline void tc_share_unlock(void) {}
#endif
...
// 1.6.6 — count of currently-used share-table slots, for diagnostic logging
// when find_or_alloc fails (Andreas debugged a silent-drop scenario where
// SYS_SHARE_SET_STR was overflowing TC_SHARE_MAX=24 with no log → the bump
// to 32 was guessed-at, not data-driven). Caller holds lock.
static int tc_share_count_used(void) {
  int used = 0;
  for (int i = 0; i < TC_SHARE_MAX; i++) {
    if (tc_share_table[i].type != TC_SHARE_TYPE_NONE) used++;
  }
  return used;
}
#endif // TC_USE_SHARE
 ...
 *
 * Ported from the Scripter ESP32_FAST_MUX feature. A hardware-timer ISR steps a small scan
 * buffer of pin-encoding bytes for jitter-free GPIO multiplexing (LED matrix / 7-segment /
 * charlieplex) far faster and steadier than the TinyC VM loop could manage. The ISR runs
 * from IRAM and uses the direct GPIO.out_w1ts / out_w1tc set/clear registers, so it keeps
 * muxing even while the main loop / flash is busy. Pins are limited to GPIO 0..31.
 *
 * Gated to the dual-core Xtensa parts — classic ESP32 (CONFIG_IDF_TARGET_ESP32) and ESP32-S3
 * (CONFIG_IDF_TARGET_ESP32S3) — where out_w1ts/out_w1tc are a plain 32-bit word for GPIO 0..31
 * and a spare HW timer + core is available. The RISC-V C-series (C3/C6/...) expose those
 * registers as a different struct (need .val) and are single-core, so they are excluded; on
 * any non-matching target fastMux() simply returns -1. Note: the original Scripter version
 * used the Arduino-core-2.x timer API (timerAlarmWrite, etc.); this port uses the core-3.x
 * API (timerBegin(freq) / timerAlarm / timerAttachInterrupt).
 *
 * Scan-buffer byte encoding (one byte per step; only the low 8 bits of each array element):
 *   bit5 (0x20) set  = COMMAND byte:
 *        bit0 (0x01) -> clear all "low"  pins   (GPIO.out_w1tc = low_pins)
 *        bit1 (0x02) -> set   all "high" pins   (GPIO.out_w1ts = high_pins)
 *        bit2 (0x04) -> retime: alarm = time * ((b & 0x1f) >> 2)   [see flash note below]
 *   bit5 (0x20) clear = SINGLE-PIN write: pin = b & 0x1f (0..31), level = (b >> 6) & 1
 *   bit7 (0x80)       = step boundary: the ISR stops after this byte until the next timer tick
 *
 * fastMux(flag, time, buf, len)  (always pass your scan-buffer array as buf, even for 1/3):
 *   flag 0 = start : configure pins from buf[] (pin = b&0x1f; initial level from b&0x40, which
 *            also builds the high_pins/low_pins masks; pinMode OUTPUT), create the timer at
 *            1 MHz (1 count = 1 us), set the period to `time` us, start. 0, or -1 on alloc fail.
 *   flag 1 = stop  : tear the timer down. Returns 0.
 *   flag 2 = load  : copy buf[] (len bytes) into the live scan sequence under lock. Returns 0.
 *   flag 3 = pos   : return the current scan index. (time/len ignored for flags 1 and 3.)
\*********************************************************************************************/
...
struct TC_FAST_PIN_MUX {
  ...
  uint8_t  scan_buff[TC_MUX_SIZE];
  uint8_t  scan_buff_size;
  uint32_t time;            // timer alarm period in microseconds (timer runs at 1 MHz)
  uint32_t low_pins;
  uint32_t high_pins;
  hw_timer_t *scan_timer;
  portMUX_TYPE scan_timerMux;
};
static TC_FAST_PIN_MUX tc_fast_pin_mux = {
  0, {0}, 0, 0, 0, 0, nullptr, portMUX_INITIALIZER_UNLOCKED
};

static void IRAM_ATTR tc_fast_mux_irq() {
  portENTER_CRITICAL_ISR(&tc_fast_pin_mux.scan_timerMux);
  if (tc_fast_pin_mux.scan_buff_size) {
    while (tc_fast_pin_mux.scan_cnt < tc_fast_pin_mux.scan_buff_size) {
      uint8_t iob = tc_fast_pin_mux.scan_buff[tc_fast_pin_mux.scan_cnt];
      if (iob & 0x20) {
        if (iob & 1) {
          GPIO.out_w1tc = tc_fast_pin_mux.low_pins;
        }
        if (iob & 2) {
          GPIO.out_w1ts = tc_fast_pin_mux.high_pins;
        }
        if (iob & 4) {
          // retime: this Arduino timer call lives in flash, not IRAM — fine in normal
          // operation; only unsafe if it happens to fire during a flash-cache-disabled
          // window. The GPIO writes above are register-direct and therefore IRAM-safe.
          uint8_t fac = (iob & 0x1f) >> 2;
          timerAlarm(tc_fast_pin_mux.scan_timer, (uint64_t)tc_fast_pin_mux.time * fac, true, 0);
        }
      } else {
      ...
      tc_fast_pin_mux.scan_cnt++;
      if (iob & 0x80) {
        break;
      }
    }
    if (tc_fast_pin_mux.scan_cnt >= tc_fast_pin_mux.scan_buff_size) {
      tc_fast_pin_mux.scan_cnt = 0;
    }
  }
  portEXIT_CRITICAL_ISR(&tc_fast_pin_mux.scan_timerMux);
}

// buf points at the resolved TinyC int32 array; the low 8 bits of each element = one mux byte.
static int32_t tc_fast_mux(uint32_t flag, uint32_t time, int32_t *buf, uint32_t len) {
  int32_t retval;
  if (flag == 0) {
    if (!buf) { return -1; }
    if (len > TC_MUX_SIZE) { len = TC_MUX_SIZE; }
    tc_fast_pin_mux.high_pins = 0;
    tc_fast_pin_mux.low_pins  = 0;
    for (uint32_t cnt = 0; cnt < len; cnt++) {
      uint8_t iob = (uint8_t)(buf[cnt] & 0xFF);
      tc_fast_pin_mux.scan_buff[cnt] = iob;
      uint8_t pin = iob & 0x1f;
      pinMode(pin, OUTPUT);
      if (iob & 0x40) {
        digitalWrite(pin, 1);
        tc_fast_pin_mux.high_pins |= ((uint32_t)1 << pin);
      } else {
        digitalWrite(pin, 0);
        tc_fast_pin_mux.low_pins |= ((uint32_t)1 << pin);
      }
    }
    tc_fast_pin_mux.scan_buff_size = 0;
    tc_fast_pin_mux.scan_cnt = 0;
    if (tc_fast_pin_mux.scan_timer) {
      timerStop(tc_fast_pin_mux.scan_timer);
      timerDetachInterrupt(tc_fast_pin_mux.scan_timer);
      timerEnd(tc_fast_pin_mux.scan_timer);
      tc_fast_pin_mux.scan_timer = nullptr;
    }
    tc_fast_pin_mux.scan_timer = timerBegin(1000000);   // 1 MHz -> 1 count = 1 us
    if (!tc_fast_pin_mux.scan_timer) {
      return -1;
    }
    tc_fast_pin_mux.time = time ? time : 1;
    timerAttachInterrupt(tc_fast_pin_mux.scan_timer, &tc_fast_mux_irq);
    timerAlarm(tc_fast_pin_mux.scan_timer, tc_fast_pin_mux.time, true, 0);
    timerStart(tc_fast_pin_mux.scan_timer);
  } else if (flag == 1) {
    if (tc_fast_pin_mux.scan_timer) {
      timerStop(tc_fast_pin_mux.scan_timer);
      timerDetachInterrupt(tc_fast_pin_mux.scan_timer);
    ...
    if (!buf) { return -1; }
    if (len > TC_MUX_SIZE) { len = TC_MUX_SIZE; }
    portENTER_CRITICAL(&tc_fast_pin_mux.scan_timerMux);
    for (uint32_t cnt = 0; cnt < len; cnt++) {
      tc_fast_pin_mux.scan_buff[cnt] = (uint8_t)(buf[cnt] & 0xFF);
    }
    tc_fast_pin_mux.scan_buff_size = len;
    portEXIT_CRITICAL(&tc_fast_pin_mux.scan_timerMux);
  } else {
    portENTER_CRITICAL(&tc_fast_pin_mux.scan_timerMux);
    retval = tc_fast_pin_mux.scan_cnt;
    portEXIT_CRITICAL(&tc_fast_pin_mux.scan_timerMux);
    return retval;
  }
  return 0;
}
#endif // USE_TINYC_FAST_MUX && (CONFIG_IDF_TARGET_ESP32 || CONFIG_IDF_TARGET_ESP32S3)
...
//
// ⚠️ This id identifies a SCALAR global and nothing else. It cannot address an
// ARRAY: bits 16-17 (the element kind) and the heap tag are both masked off by
// the 0x0FFF, so a heap-allocated array arrives here as its bare handle and is
// indistinguishable from global index 0. webText is the only widget that writes
// an array, and it therefore does NOT round-trip through this id -- it sends the
// whole ref instead (see SYS_WEB_TEXT and the "S_" branch of TinyC_WebSetVar).
...
static int tc_syscall(TcVM *vm, uint16_t id) {
  ...
  switch (id) {
    ...
    case SYS_DIGITAL_WRITE:
      ...
      break;
    case SYS_DMX_WRITE:
      b = TC_POP(vm); a = TC_POP(vm);   // a=channel(1..), b=value(0..255)
      tc_dmx_set(a, b);
      break;
    case SYS_DMX_INIT: {
      int dg = TC_POP(vm);
    ...
    case SYS_MTR_NAME: {                         // matterName(ep, name) — bridge endpoint label
      a = TC_POP(vm);                            // name ref (pushed last): string literal OR char[]
      int32_t ep = TC_POP(vm);
      char nm[40];
      tc_ref_to_cstr(vm, a, nm, sizeof(nm));     // handles const-pool literals AND heap/local char[]
      if (mtrc_ensure_inited()) matter_set_label((uint16_t)ep, nm);
      break;
    }
    ...
    case SYS_MTR_NAME:    TC_POP(vm); TC_POP(vm); break;
    ...
    // ── Serial in Bloecken: ein Aufruf statt einer je Byte ────────────────
    // Wort fuer Wort wie SYS_USB_READ/WRITE: die VM haelt EIN int32 je
    // Element, auch fuer Bytes, also geht alles ueber einen Zwischenpuffer.
    case SYS_SERIAL_READ_ARR: {
      // serialReadArray(h, arr, n) -> bytes read
      int32_t n   = TC_POP(vm);   // count
      int32_t ref = TC_POP(vm);   // array
      int32_t hh  = TC_POP(vm);   // handle
      TasmotaSerial *p = tc_serial_get(hh);
      if (!p || n <= 0) { TC_PUSH(vm, 0); break; }
      // ⚠️ Never write past the array -- `n` comes from the program.
      int32_t kap = tc_ref_maxlen(vm, ref);
      if (n > kap) { n = kap; }
      // ⚠️ Ask available() ONCE, not per byte: that second call per byte is
      // the very reason this syscall exists.
      int32_t da = p->available();
      if (da < n) { n = da; }
      // ⚠️⚠️ Do NOT write into the array by hand. The first attempt did
      // `buf[i] = byte` through tc_resolve_ref(): right for an int32 array and
      // WRONG for a packed byte[], where four bytes share one slot. The two
      // kinds are told apart explicitly below.
      TC_UBUF(tmp, 256);
      int32_t gesamt = 0;
      while (gesamt < n) {
        ...
        while (k < teil) {
          int c = p->read();
          if (c < 0) { break; }          // nothing left -- do NOT wait
          tmp[k] = (uint8_t)(c & 0xFF);
          k++;
        }
        if (k <= 0) { break; }
        // ⚠️ Write back at offset `gesamt`. tc_ref_put_bytes() has no offset
        // argument, so the two array kinds are handled here directly rather
        // than by moving the ref (which would be a different array).
        if (tc_ref_is_bytes(ref)) {
          int32_t *basis = tc_resolve_ref(vm, ref);
          if (!basis) { break; }
          memcpy((uint8_t *)basis + gesamt, tmp, (size_t)k);
        } else {
          int32_t *basis = tc_resolve_ref(vm, ref);
          if (!basis) { break; }
          for (int32_t i = 0; i < k; i++) { basis[gesamt + i] = (int32_t)tmp[i]; }
        }
        gesamt += k;
        if (k < teil) { break; }
      }
    ...
    case SYS_SERIAL_WRITE_BUF: {
      ...
      TasmotaSerial *p = tc_serial_get(hh);
      // ⚠️⚠️ This used to read `b <= 256`, and longer meant writing NOTHING
      // AT ALL. No error, no return value (the function returns void), just
      // silence -- hand a 1024-byte block from a bridge to it and the whole
      // block disappears, with the wire the obvious place to look. It now
      // writes in chunks, as many as it takes.
      if (p && b > 0) {
        const int32_t kap = tc_ref_maxlen(vm, a);
        if (b > kap) { b = kap; }
        if (tc_ref_is_bytes(a)) {
          // Packed byte[]: the bytes already sit that way in memory, so they
          // go out in ONE write -- no length limit.
          int32_t *basis = tc_resolve_ref(vm, a);
          if (basis && b > 0) { p->write((uint8_t *)basis, (size_t)b); }
        } else {
          // int32 array: gather the low bytes, chunk by chunk.
          int32_t *basis = tc_resolve_ref(vm, a);
          if (basis) {
            TC_UBUF(tbuf, 256);
            int32_t ab = 0;
            while (ab < b) {
              int32_t teil = b - ab;
              if (teil > (int32_t)sizeof(tbuf)) { teil = sizeof(tbuf); }
              for (int32_t k = 0; k < teil; k++) {
                tbuf[k] = (uint8_t)(basis[ab + k] & 0xFF);
              }
              p->write(tbuf, (size_t)teil);
              ab += teil;
            }
          }
        }
    ...
    case SYS_TASM_CMD: {
      ...
      for (int32_t i = 0; i < rlen; i++) {
        tc_chr_put(buf, ist_bytes, i, (int32_t)(uint8_t)resp[i]);
      }
      tc_chr_put(buf, ist_bytes, rlen, 0);  // null-terminate
    ...
    case SYS_STRLEN: {
      ...
      if (cs) {
        ...
        if (p) { len = tc_str_len_b(p, tc_ref_is_bytes(a), tc_ref_maxlen(vm, a)); }
      }
      TC_PUSH(vm, len);
      break;
    }
    case SYS_STRCPY: {
      ...
      if (!dst) break;
      const bool dib = tc_ref_is_bytes(dst_ref);   // dst may be a packed byte[]
      int32_t max = tc_ref_maxlen(vm, dst_ref) - 1;
      // Source may be a const-pool string ref (e.g. string literal passed
      // through a function param), a char[] or a byte[]; all read byte-aware.
      if (tc_is_const_ref(src_ref)) {
        const char *s = tc_const_ref_str(vm, src_ref);
        int32_t i = 0;
        ...
        if (src) {
          const bool sib = tc_ref_is_bytes(src_ref);
          int32_t i = 0;
    ...
    case SYS_STRCAT: {
      ...
      int32_t *dst = tc_resolve_ref(vm, dst_ref);
      if (!dst) break;
      const bool dib = tc_ref_is_bytes(dst_ref);
      int32_t max = tc_ref_maxlen(vm, dst_ref) - 1;
      int32_t di = tc_str_len_b(dst, dib, max + 1);
      if (di > max) di = max;
    ...
    case SYS_STRCMP: {
      ...
      const bool aib = (!ca) && tc_ref_is_bytes(a_ref);
      const bool bib = (!cb) && tc_ref_is_bytes(b_ref);
      int32_t result = 0;
      if ((ca || pa) && (cb || pb)) {
        ...
        int32_t i = 0;
        while (1) {
          int32_t va = ca ? (int32_t)(uint8_t)ca[i] : (i < max_a ? (tc_chr_get(pa, aib, i) & 0xFF) : 0);
          int32_t vb = cb ? (int32_t)(uint8_t)cb[i] : (i < max_b ? (tc_chr_get(pb, bib, i) & 0xFF) : 0);
    ...
    case SYS_STRCMP_CONST: {
      ...
      if (pa && ci >= 0 && ci < vm->const_count && vm->constants[ci].type == 1) {
        ...
        const bool aib = tc_ref_is_bytes(a_ref);
        int32_t max_a = tc_ref_maxlen(vm, a_ref);
        int32_t i = 0;
        while (i < max_a && tc_chr_get(pa, aib, i) != 0 && pb[i] != 0
               && tc_chr_get(pa, aib, i) == (int32_t)(uint8_t)pb[i]) i++;
        int32_t av = (i < max_a) ? tc_chr_get(pa, aib, i) : 0;
        int32_t bv = (int32_t)(uint8_t)pb[i];
        result = av - bv;
      }
      TC_PUSH(vm, result);
      break;
    ...
    case SYS_STR_PRINT: {
      ...
      if (cs) {
        ...
        int32_t *p = tc_resolve_ref(vm, a);
        if (p) {
          const bool ib = tc_ref_is_bytes(a);
          int32_t max = tc_ref_maxlen(vm, a);
          int32_t i = 0;
          while (i < max) {
            int32_t c = tc_chr_get(p, ib, i);
            if (c == 0) break;
            tc_output_char((char)(c & 0xFF));
            i++;
          }
        }
      }
    ...
    case SYS_FILE_READ: {
      ...
      while (total < maxBytes) {
        ...
        if (n <= 0) break;
        for (int i = 0; i < n; i++) {
          // Byte-aware, the read counterpart of SYS_FILE_WRITE above.
          tc_chr_put(buf, tc_ref_is_bytes(buf_ref), total + i, (int32_t)tmp[i]);
        }
        total += n;
      }
    ...
    case SYS_FILE_WRITE: {
      ...
      // Byte-aware read side: tc_ref_maxlen() gives the element count either
      // way, but a packed byte[] holds four elements per slot — reading it as
      // int32 slots walked off the end and wrote garbage into the file.
      const bool ist_bytes = tc_ref_is_bytes(buf_ref);
      int32_t maxSlots = tc_ref_maxlen(vm, buf_ref);
      if (len > maxSlots) len = maxSlots;
      // Convert array elements to bytes and write
      TC_UBUF(tmp, 256);
      int32_t total = 0;
      while (total < len) {
        ...
        for (int32_t i = 0; i < chunk; i++) {
          tmp[i] = (uint8_t)(tc_chr_get(buf, ist_bytes, total + i) & 0xFF);
        }
    ...
    // fileRename(from, to) — rename/move a file. Did not exist before; without
    // it, "done once" states had to be modelled with extra marker files or by
    // deleting the original (gemu 2026-07-25, while reworking the sml_chart
    // storage). LittleFS/FFat do it natively. The target is NOT overwritten
    // silently: if it exists the call returns -1 -- otherwise a typo in the
    // target name could destroy data.
    // ── FTDI am USB-Host ──────────────────────────────────────────────────
    case SYS_USB_INIT:
    case SYS_USB_STATE:
    case SYS_USB_AVAILABLE:
    case SYS_USB_CLOSE:
    case SYS_USB_DEINIT: {
#ifdef USE_TINYC_USBSERIAL
      if (id == SYS_USB_INIT)            { TC_PUSH(vm, TcUsbInit() ? 1 : 0); }
      else if (id == SYS_USB_STATE)      { TC_PUSH(vm, TcUsb.state); }
      else if (id == SYS_USB_AVAILABLE)  { TC_PUSH(vm, TcUsbAvailable()); }
      else if (id == SYS_USB_CLOSE)      { TcUsbClose(); TC_PUSH(vm, 0); }
      else                               { TcUsbDeinit(); TC_PUSH(vm, 0); }
#else
      TC_PUSH(vm, 0);
#endif
      break;
    ...
    case SYS_USB_OPEN: {
      ...
      TC_PUSH(vm, TcUsbOpen((uint32_t)baud) ? 1 : 0);
#else
      TC_POP(vm);
      TC_PUSH(vm, 0);
#endif
    ...
    case SYS_USB_INFO: {
      ...
      int32_t w = 0;
      switch (sel) {
        case 0: w = TcUsb.vid; break;
        case 1: w = TcUsb.pid; break;
        case 2: w = (int32_t)TcUsb.rx; break;
        case 3: w = (int32_t)TcUsb.tx; break;
        case 4: w = (int32_t)TcUsb.verloren; break;
        default: w = 0; break;
      }
      TC_PUSH(vm, w);
#else
    ...
    case SYS_USB_WRITE: {
      ...
      int32_t *buf = tc_resolve_ref(vm, ref);
      if (!buf || n <= 0) { TC_PUSH(vm, (id == SYS_USB_READ) ? 0 : -1); break; }
      int32_t kap = tc_ref_maxlen(vm, ref);
      if (n > kap) { n = kap; }
      TC_UBUF(tmp, 256);
      int32_t gesamt = 0;
      while (gesamt < n) {
        ...
        if (teil > (int32_t)sizeof(tmp)) { teil = sizeof(tmp); }
        if (id == SYS_USB_READ) {
          uint16_t got = TcUsbPull(tmp, (uint16_t)teil);
          if (!got) { break; }                        // nichts mehr da — NICHT warten
          for (uint16_t i = 0; i < got; i++) { buf[gesamt + i] = (int32_t)tmp[i]; }
          gesamt += got;
        } else {
          for (int32_t i = 0; i < teil; i++) { tmp[i] = (uint8_t)(buf[gesamt + i] & 0xFF); }
          if (TcUsbWrite(tmp, (uint16_t)teil) < 0) { gesamt = -1; break; }
          gesamt += teil;
        }
      }
      TC_PUSH(vm, gesamt);
#else
      TC_POP(vm); TC_POP(vm);
      TC_PUSH(vm, (id == SYS_USB_READ) ? 0 : -1);
#endif
      break;
    }
    ...
    case SYS_SPP_CLOSE: {
      ...
      // 4 = the last connect attempt FAILED (peer refused, out of range, slot taken).
      // Widening the range is safe: every script waits for 3, so 4 behaves exactly like
      // the old 1 for them -- but a script that knows about it can give up in seconds
      // instead of sitting out its whole timeout.
      else if (id == SYS_SPP_STATE)     { TC_PUSH(vm, (TC_SPP_BEREIT == TcSpp.state &&
                                                       TcSpp.verbinde_fehlgeschlagen) ? 4
                                                                                      : TcSpp.state); }
      else if (id == SYS_SPP_AVAILABLE) { TC_PUSH(vm, TcSppAvailable()); }
      else if (id == SYS_SPP_CLOSE)     { TcSppClose(); TC_PUSH(vm, 0); }
      else                              { TcSppDeinit(); TC_PUSH(vm, 0); }
#else
    ...
    case SYS_FILE_RENAME_REF: {
      ...
      // Source and target have to live on the same filesystem -- rename()
      // cannot copy across a filesystem boundary (flash <-> SD).
      if (!fsf || !fst || fsf != fst) {
        AddLog(LOG_LEVEL_DEBUG, PSTR("TCC: fileRename(\"%s\",\"%s\") -> -1 (different filesystem)"), pfrom, pto);
        TC_PUSH(vm, -1);
        break;
      }
      if (!fsf->exists(pfrom)) { TC_PUSH(vm, -1); break; }
      if (fsf->exists(pto))    { TC_PUSH(vm, -1); break; }   // never overwrite silently
    ...
    case SYS_FILE_READ_ARR: {
#ifdef USE_UFILESYS
      // Read tab/comma-delimited FLOAT values into the array (Scripter fra()
      // compatible). Streams value-by-value (no fixed line buffer) so arrays of
      // any size work. `count` caps how many to read (-1 = up to the array's
      // capacity); reading also stops at EOF. Returns elements read.
      ...
      if (handle >= 0 && handle < TC_MAX_FILE_HANDLES && Tinyc->file_used[handle]) {
        ...
        if (count >= 0 && count < cap) cap = count;   // explicit cap (small global arrays)
        if (base && cap > 0) {
          char tok[32];
          int ti = 0;
          while (n < cap) {
            int c = f.read();
            if (c < 0) {                            // EOF — flush any pending token
              if (ti > 0) { tok[ti] = 0; float fv = atof(tok); memcpy(&base[n++], &fv, 4); }
              break;
            }
            if (c == '\t' || c == ',' || c == '\n' || c == '\r' || c == ' ') {
              if (ti > 0) { tok[ti] = 0; float fv = atof(tok); memcpy(&base[n++], &fv, 4); ti = 0; }
    ...
    case SYS_FILE_WRITE_ARR: {
#ifdef USE_UFILESYS
      // Write `count` array elements as tab-separated FLOAT text (Scripter fwa()
      // compatible), one array per line. Explicit count (like fileWriteBin) so
      // small GLOBAL arrays write the right length — tc_ref_maxlen over-reports
      // for globals (it returns the whole global-pool remainder). `decimals`
      // caps decimal places (trailing zeros stripped, like Scripter precision)
      // to keep the text compact; `append`!=0 keeps the line open (trailing tab)
      // so several arrays can share one line.
      ...
      if (handle >= 0 && handle < TC_MAX_FILE_HANDLES && Tinyc->file_used[handle]) {
        File &f = tc_file_handles[handle];
        int32_t *base = tc_resolve_ref(vm, ref);
        // A packed array holds INTEGERS, not float bit patterns, and holds more
        // of them than it has slots -- so both the count and the decoding come
        // off the element kind. Without this an int16[] chart ring exported as
        // bit-cast floats: garbage values, and only half the samples.
        const int wa_kind = tc_ref_elem_kind(ref);
        int32_t cap   = tc_ref_elem_count(vm, ref);
        int32_t n = count;
        if (n > cap) n = cap;          // never read past the array's storage
        if (n < 0) n = 0;
        if (base && n > 0) {
          char tmp[32];
          for (int32_t i = 0; i < n; i++) {
            ...
            float fv;
            switch (wa_kind) {
              case TC_ELEM_BYTE: fv = (float)((const uint8_t  *)base)[i]; break;
              case TC_ELEM_I16:  fv = (float)((const int16_t  *)base)[i]; break;
              case TC_ELEM_U16:  fv = (float)((const uint16_t *)base)[i]; break;
              default: memcpy(&fv, &base[i], sizeof(float)); break;
            }
            // negative precision = up to `decimals` places, trailing zeros stripped
            ext_snprintf_P(tmp, sizeof(tmp), PSTR("%*_f"), -decimals, &fv);
            f.write((const uint8_t*)tmp, strlen(tmp));
          }
          f.write((uint8_t)(append ? '\t' : '\n'));
          f.flush();
        }
      }
#else
    ...
    case SYS_FILE_READ_BIN: {
#ifdef USE_UFILESYS
      // (handle, arr_ref, count) -> elements_read (-1 = err)
      // Reads up-to `count` int32 elements packed as 4-byte little-endian.
      // Same syscall serves int[] and float[] — both are int32 in memory,
      // and the on-disk bit pattern is identical. The script knows the
      // type; this just moves bits.
      int32_t count   = TC_POP(vm);
      ...
      int32_t *base = tc_resolve_ref(vm, buf_ref);
      if (!base) { TC_PUSH(vm, -1); break; }
      int32_t maxSlots = tc_ref_maxlen(vm, buf_ref);   // byte[] -> bytes, else slots
      if (count < 0) count = 0;
      if (count > maxSlots) count = maxSlots;
      File &f = tc_file_handles[handle];
      // A byte[] target reads RAW bytes (one file byte per element), not the
      // 4-byte-LE int32 elements below — `count` is a byte count here.
      if (tc_ref_is_bytes(buf_ref)) {
        int n = (count > 0) ? f.read((uint8_t*)base, count) : 0;
        if (n < 0) n = 0;
        AddLog(LOG_LEVEL_DEBUG, PSTR("TCC: fileReadBin(%d, %d) -> %d bytes"), handle, count, n);
        TC_PUSH(vm, n);
        break;
      }
      uint8_t tmp[256];           // 64 int32 elements per chunk
      int32_t total_elems = 0;
      int32_t bytes_remaining = count * 4;
      while (bytes_remaining > 0) {
        int32_t chunk = bytes_remaining;
        if (chunk > (int32_t)sizeof(tmp)) chunk = sizeof(tmp);
        chunk &= ~3;              // round to whole-element boundary
        if (chunk == 0) break;
        int n = f.read(tmp, chunk);
        if (n <= 0) break;
        int n_elems = n / 4;      // discard any tail short of 4 bytes
        for (int i = 0; i < n_elems; i++) {
          int32_t v = ((int32_t)tmp[i*4]      )
                    | ((int32_t)tmp[i*4 + 1] << 8)
                    | ((int32_t)tmp[i*4 + 2] << 16)
                    | ((int32_t)tmp[i*4 + 3] << 24);
          base[total_elems + i] = v;
        }
        total_elems     += n_elems;
        bytes_remaining -= n_elems * 4;
        if (n < chunk) break;     // short read = EOF
      }
      AddLog(LOG_LEVEL_DEBUG, PSTR("TCC: fileReadBin(%d, %d) -> %d elems"),
             handle, count, total_elems);
      TC_PUSH(vm, total_elems);
#else
      TC_POP(vm); TC_POP(vm); TC_POP(vm);
      TC_PUSH(vm, -1);
#endif
    ...
    case SYS_FILE_WRITE_BIN: {
#ifdef USE_UFILESYS
      // (handle, arr_ref, count) -> elements_written (-1 = err)
      // Writes `count` int32 elements packed as 4-byte little-endian.
      int32_t count   = TC_POP(vm);
      int32_t buf_ref = TC_POP(vm);
      int32_t handle  = TC_POP(vm);
      if (handle < 0 || handle >= TC_MAX_FILE_HANDLES || !Tinyc->file_used[handle]) {
      ...
      if (count > maxSlots) count = maxSlots;
      File &f = tc_file_handles[handle];
      // A byte[] source writes RAW bytes (one per element), not 4-byte-LE
      // int32 elements — `count` is a byte count here.
      if (tc_ref_is_bytes(buf_ref)) {
        int n = (count > 0) ? f.write((uint8_t*)base, count) : 0;
        f.flush();
        if (n < 0) n = 0;
        AddLog(LOG_LEVEL_DEBUG, PSTR("TCC: fileWriteBin(%d, %d) -> %d bytes"), handle, count, n);
        TC_PUSH(vm, n);
        break;
      }
      uint8_t tmp[256];           // 64 elements per chunk
      int32_t total_elems = 0;
      int32_t remaining = count;
      while (remaining > 0) {
        int32_t chunk_elems = remaining;
        if (chunk_elems > 64) chunk_elems = 64;
        for (int32_t i = 0; i < chunk_elems; i++) {
          int32_t v = base[total_elems + i];
          tmp[i*4]     = v & 0xFF;
          tmp[i*4 + 1] = (v >> 8)  & 0xFF;
          tmp[i*4 + 2] = (v >> 16) & 0xFF;
          tmp[i*4 + 3] = (v >> 24) & 0xFF;
        }
        int n = f.write(tmp, chunk_elems * 4);
        if (n <= 0) break;
        total_elems += chunk_elems;
        remaining   -= chunk_elems;
        if (n < (int)(chunk_elems * 4)) break;   // short write
      }
      f.flush();
      AddLog(LOG_LEVEL_DEBUG, PSTR("TCC: fileWriteBin(%d, %d) -> %d elems"),
             handle, count, total_elems);
      TC_PUSH(vm, total_elems);
#else
      TC_POP(vm); TC_POP(vm); TC_POP(vm);
    ...
    case SYS_UDP_FUNC: {
      ...
      switch (mode) {
          ...
          if (Tinyc->udp_port_open && !TasmotaGlobal.global_state.network_down) {
            ...
            if (plen > 0) {
              ...
              if (len > 0) {
                packet[len] = 0;
                int32_t *buf = tc_resolve_ref(vm, buf_ref);
                int32_t maxLen = tc_ref_maxlen(vm, buf_ref);   // byte[] -> bytes, else slots
                if (buf) {
                  const bool is_bytes = tc_ref_is_bytes(buf_ref);
                  int n = (len < maxLen - 1) ? len : maxLen - 1;
                  if (is_bytes) {
                    for (int i = 0; i < n; i++) ((uint8_t*)buf)[i] = (uint8_t)packet[i];
                    ((uint8_t*)buf)[n] = 0;
                  } else {
        ...
        case 7: { // udp(7, url, port, arr, count) → send array bytes to url:port
          ...
          char url[64];
          tc_ref_to_cstr(vm, url_ref, url, sizeof(url));
          int32_t maxLen = tc_ref_maxlen(vm, arr_ref);   // byte[] -> bytes, else slots
          if (count > 0 && maxLen > 0) {
            if (count > maxLen) count = maxLen;
            if (count > 512) count = 512;
            uint8_t *payload = (uint8_t*)malloc(count);
            if (payload) {
              // accepts char[] OR byte[] via tc_ref_bytes.
              int32_t n = 0;
              uint8_t *bp = tc_ref_bytes(vm, arr_ref, count, payload, count, &n);
              if (bp && n > 0) {
                IPAddress adr;
                adr.fromString(url);
                WiFiUDP tmpUdp;
        ...
        case 10: { // udp(10, mcast_ip_ref) → drop multicast membership, keep socket open as unicast
          // 1.6.6 — Rewritten after Andreas's A/B test (15.05.) showed the
          // 1.6.1-shipped raw-LwIP `igmp_leavegroup()` path returned ERR_OK
          // but multicast packets kept arriving at the script (spw_pkts/h
          // identical in Phase A vs B). Root cause: Arduino-ESP32
          // `NetworkUDP::beginMulticast()` joins via SOCKET-layer
          // `setsockopt(IPPROTO_IP, IP_ADD_MEMBERSHIP)` — which writes BOTH
          // the netif IGMP table AND a per-socket membership list in
          // lwip_sockets. Our `igmp_leavegroup()` only updated the netif
          // table; the socket-level membership remained, so the UDP demux
          // continued to deliver matching packets to the PCB.
          //
          // Fix: use `NetworkUDP::stop()` which calls
          // `setsockopt(IP_DROP_MEMBERSHIP)` on the underlying fd — this
          // clears both layers atomically. Then re-`begin(port)` as a plain
          // unicast listener on the same port so the socket stays usable
          // for non-multicast UDP traffic on that port (Andreas's
          // "drop without socket-teardown" requirement is satisfied at the
          // user-visible level: the port keeps accepting unicast packets).
          //
          // `mcast_ip` arg is kept for API compatibility and validation only —
          // the actual drop targets whatever multicast IP was joined via
          // udp(9). A mismatch logs a warning but still performs the drop
          // (defensive: the user might call udp(10) without knowing the
          // currently-bound IP).
          //
          // Returns 1 on success, 0 on bad-IP / socket-not-open / reopen-fail.
          // ESP8266: same code path now works because we no longer touch the
          // raw LwIP IGMP API — the Arduino socket layer handles everything.
          int32_t ip_ref = TC_POP(vm);
          char ip_str[32];
          tc_ref_to_cstr(vm, ip_ref, ip_str, sizeof(ip_str));
          IPAddress mcast;
          if (!mcast.fromString(ip_str)) {
            AddLog(LOG_LEVEL_INFO, PSTR("TCC: udp(10) bad IP '%s'"), ip_str);
            TC_PUSH(vm, 0);
            break;
          }
          if (!Tinyc->udp_port_open || Tinyc->udp_port_num == 0) {
            AddLog(LOG_LEVEL_INFO, PSTR("TCC: udp(10) socket not open"));
            TC_PUSH(vm, 0);
            break;
          }
          ...
          if ((uint32_t)Tinyc->udp_port_mcast != 0 && Tinyc->udp_port_mcast != mcast) {
            char bound[32];
            snprintf(bound, sizeof(bound), "%u.%u.%u.%u",
              ((uint32_t)Tinyc->udp_port_mcast)      & 0xFF,
              ((uint32_t)Tinyc->udp_port_mcast >> 8) & 0xFF,
              ((uint32_t)Tinyc->udp_port_mcast >>16) & 0xFF,
              ((uint32_t)Tinyc->udp_port_mcast >>24) & 0xFF);
            AddLog(LOG_LEVEL_INFO, PSTR("TCC: udp(10) IP arg %s != bound %s — dropping bound anyway"),
                   ip_str, bound);
          }
          uint16_t saved_port = Tinyc->udp_port_num;
          Tinyc->udp_port.stop();                // setsockopt(IP_DROP_MEMBERSHIP) + close fd
          Tinyc->udp_port_mcast = IPAddress(0,0,0,0);  // prevent case-1 watchdog rejoin
          bool ok = Tinyc->udp_port.begin(saved_port);  // reopen as plain unicast listener
          if (ok) {
            Tinyc->udp_port_last_rx = millis();
            AddLog(LOG_LEVEL_INFO, PSTR("TCC: udp(10) dropped %s, port %d reopened as unicast"),
                   ip_str, saved_port);
            TC_PUSH(vm, 1);
          } else {
            Tinyc->udp_port_open = false;
            AddLog(LOG_LEVEL_INFO, PSTR("TCC: udp(10) drop %s ok but port %d reopen failed"),
                   ip_str, saved_port);
            TC_PUSH(vm, 0);
          }
          break;
    ...
    case SYS_I2C_READ_BUF: {
      ...
      if (arr && len > 0) {
        ...
        if (!err) {  // I2cReadBuffer returns 0=OK, 1=Error
          tc_ref_put_bytes(vm, buf_ref, tmpbuf, len);   // byte[] packs, int32 expands
          TC_PUSH(vm, 1);
        } else {
          TC_PUSH(vm, 0);
        }
    ...
    case SYS_I2C_READ_RS: {
      ...
      int32_t *arr = tc_resolve_ref(vm, buf_ref);
      int32_t maxLen = tc_ref_maxlen(vm, buf_ref);   // byte[] -> bytes, else slots
      const bool rs_is_bytes = tc_ref_is_bytes(buf_ref);
      if (arr && len > 0) {
        if (len > maxLen) len = maxLen;
        if (len > 255) len = 255;
        ...
        if (myWire.endTransmission(false) == 0) {  // repeated START
          if ((uint8_t)len == myWire.requestFrom((uint8_t)a, (uint8_t)len)) {
            for (int32_t i = 0; i < len; i++) {
              int v = myWire.read();
              if (rs_is_bytes) ((uint8_t*)arr)[i] = (uint8_t)v;
              else             arr[i] = (int32_t)v;
            }
            TC_PUSH(vm, 1);
          } else {
            TC_PUSH(vm, 0);
    ...
    case SYS_I2C_WRITE_BUF: {
      ...
      if (len > 0 && maxLen > 0) {
        ...
        // accepts char[] OR byte[] via tc_ref_bytes.
        int32_t n = 0;
        uint8_t *bp = tc_ref_bytes(vm, buf_ref, len, tmpbuf, sizeof(tmpbuf), &n);
        if (bp && n > 0) {
          TC_PUSH(vm, I2cWriteBuffer((uint8_t)a, (uint8_t)b, bp, (uint16_t)n, (uint8_t)bus) ? 0 : 1);  // 0=OK,1=Err
        } else {
          TC_PUSH(vm, 0);
        }
      } else {
    ...
    case SYS_RESPONSE_CMND: {
      ...
      if (arr && maxLen > 0) {
        char tmpbuf[TC_RESPONSE_MAX];
        int32_t cap = TC_RESPONSE_MAX - 1;
        int32_t n = (maxLen < cap) ? maxLen : cap;
        int32_t i;
        for (i = 0; i < n && arr[i]; i++) { tmpbuf[i] = (char)(arr[i] & 0xFF); }
        tmpbuf[i] = '\0';
        // Don't fail silently: if we stopped because the cap was hit
        // (not a NUL), the JSON is truncated → Tasmota shows empty {}.
        // Same anti-pattern as the 1.6.6 shareSet* cap log.
        if (i == cap && arr[i]) {
          AddLog(LOG_LEVEL_INFO,
                 PSTR("TCC: responseCmnd output truncated at %d chars "
                      "(raise TC_RESPONSE_MAX in user_config_override.h)"),
                 cap);
        }
        if (tmpbuf[0] == '{') {
    ...
    case SYS_SML_GETSTR: {
      ...
      if (sval && ref) {
        ...
        if (buf && maxLen > 0) {
          // Byte-aware: tc_ref_maxlen() answers in BYTES for a packed byte[],
          // so a bare buf[i] would write four times the array's size.
          const bool ist_bytes = tc_ref_is_bytes(ref);
          int slen = strlen(sval);
          if (slen >= maxLen) slen = maxLen - 1;
          for (int i = 0; i < slen; i++) tc_chr_put(buf, ist_bytes, i, (int32_t)(uint8_t)sval[i]);
    ...
    case SYS_STR_TOKEN: {
      ...
      if (src && dst && n >= 1) {
        ...
        int32_t si = 0, idx = 1;
        while (idx < n && si < src_max && tc_chr_get(src, sib, si) != 0) {
          if (tc_chr_get(src, sib, si) == delim) idx++;
          si++;
        }
        if (idx == n && si < src_max && tc_chr_get(src, sib, si) != 0) {
          int32_t di = 0, c;
          while (si < src_max && (c = tc_chr_get(src, sib, si)) != 0 && c != delim && di < dst_max) {
            tc_chr_put(dst, dib, di++, c); si++;
          }
          tc_chr_put(dst, dib, di, 0);
          result = di;
        } else {
          tc_chr_put(dst, dib, 0, 0);
        }
      }
      TC_PUSH(vm, result);
      break;
    ...
    case SYS_STR_SUB: {
      ...
      int32_t result = 0;
      if ((sc || src) && dst) {
        const bool sib = (!sc) && tc_ref_is_bytes(src_ref);
        const bool dib = tc_ref_is_bytes(dst_ref);
        int32_t src_max = sc ? 0x7FFFFFFF : tc_ref_maxlen(vm, src_ref);
        int32_t dst_max = tc_ref_maxlen(vm, dst_ref) - 1;
        #define TC_SC(i) (sc ? (int32_t)(uint8_t)sc[i] : ((i) < src_max ? (tc_chr_get(src, sib, i) & 0xFF) : 0))
        ...
        for (int32_t i = 0; i < slen; i++) {
          tc_chr_put(dst, dib, i, TC_SC(pos + i));
        }
        tc_chr_put(dst, dib, slen, 0);
        result = slen;
        #undef TC_SC
      }
      TC_PUSH(vm, result);
    ...
    case SYS_STR_FIND: {
      ...
      if ((hc || haystack) && (nc || needle)) {
        const bool hib = (!hc) && tc_ref_is_bytes(haystack_ref);
        const bool nib = (!nc) && tc_ref_is_bytes(needle_ref);
        int32_t hmax = hc ? 0x7FFFFFFF : tc_ref_maxlen(vm, haystack_ref);
        int32_t nmax = nc ? 0x7FFFFFFF : tc_ref_maxlen(vm, needle_ref);
        #define TC_HC(i) (hc ? (int32_t)(uint8_t)hc[i] : ((i) < hmax ? (tc_chr_get(haystack, hib, i) & 0xFF) : 0))
        #define TC_NC(i) (nc ? (int32_t)(uint8_t)nc[i] : ((i) < nmax ? (tc_chr_get(needle, nib, i)   & 0xFF) : 0))
    ...
    case SYS_STR_FIND_CONST: {
      ...
      if (haystack && ci >= 0 && ci < vm->const_count && vm->constants[ci].type == 1) {
        ...
        const bool hib = tc_ref_is_bytes(haystack_ref);
        int32_t hmax = tc_ref_maxlen(vm, haystack_ref);
        int32_t hlen = tc_str_len_b(haystack, hib, hmax);
        int32_t nlen = strlen(needle);
        if (nlen > 0 && nlen <= hlen) {
          for (int32_t i = 0; i <= hlen - nlen; i++) {
            bool match = true;
            for (int32_t j = 0; j < nlen; j++) {
              if (tc_chr_get(haystack, hib, i + j) != (int32_t)(uint8_t)needle[j]) { match = false; break; }
            }
            if (match) { result = i; break; }
          }
        }
    ...
    // ── String ops 1.5.0 ──────────────────────────────────────────────────
    case SYS_STR_REPLACE_CONST: {
      // strReplace(arr, "old", "new") — in-place replace ALL occurrences.
      // Returns number of replacements. Result is null-terminated; if the
      // expansion would overflow the buffer, stops cleanly at the limit.
      int32_t new_ci = TC_POP(vm);
      int32_t old_ci = TC_POP(vm);
      int32_t arr_ref = TC_POP(vm);
      int32_t *arr = tc_resolve_ref(vm, arr_ref);
      int32_t count = 0;
      if (arr && old_ci >= 0 && old_ci < vm->const_count && vm->constants[old_ci].type == 1
              && new_ci >= 0 && new_ci < vm->const_count && vm->constants[new_ci].type == 1) {
        const char *oldstr = vm->constants[old_ci].str.ptr;
        const char *newstr = vm->constants[new_ci].str.ptr;
        const bool ib = tc_ref_is_bytes(arr_ref);      // packed byte[] or int32
        int32_t maxlen = tc_ref_maxlen(vm, arr_ref);
        int32_t oldlen = strlen(oldstr);
        int32_t newlen = strlen(newstr);
        if (oldlen > 0) {
          // Compute current length
          int32_t curlen = tc_str_len_b(arr, ib, maxlen);
          // Walk and replace. After each replacement, shift tail.
          int32_t i = 0;
          while (i + oldlen <= curlen) {
            // Match?
            bool match = true;
            for (int32_t j = 0; j < oldlen; j++) {
              if (tc_chr_get(arr, ib, i + j) != (int32_t)(uint8_t)oldstr[j]) { match = false; break; }
            }
            if (!match) { i++; continue; }
            // Compute new total length after this single replacement
            int32_t delta = newlen - oldlen;
            int32_t new_curlen = curlen + delta;
            if (new_curlen >= maxlen) {
              // Would overflow buffer — stop, leave the string as-is up to here.
              break;
            }
            // Shift tail (arr[i+oldlen..curlen]) to arr[i+newlen..]
            if (delta > 0) {
              for (int32_t k = curlen; k >= i + oldlen; k--) tc_chr_put(arr, ib, k + delta, tc_chr_get(arr, ib, k));
            } else if (delta < 0) {
              for (int32_t k = i + oldlen; k <= curlen; k++) tc_chr_put(arr, ib, k + delta, tc_chr_get(arr, ib, k));
            }
            // Write new substring
            for (int32_t j = 0; j < newlen; j++) tc_chr_put(arr, ib, i + j, (int32_t)(uint8_t)newstr[j]);
            curlen = new_curlen;
            i += newlen;
            count++;
          }
          // Null-terminate (defensive — shifts above keep the original \0 in place)
          if (curlen < maxlen) tc_chr_put(arr, ib, curlen, 0);
        }
      }
      TC_PUSH(vm, count);
      break;
    }
    case SYS_STR_STARTS_CONST: {
      // strStartsWith(arr, "prefix") -> 1 if arr begins with prefix, else 0
      int32_t ci = TC_POP(vm);
      int32_t arr_ref = TC_POP(vm);
      int32_t *arr = tc_resolve_ref(vm, arr_ref);
      int32_t result = 0;
      if (arr && ci >= 0 && ci < vm->const_count && vm->constants[ci].type == 1) {
        const char *prefix = vm->constants[ci].str.ptr;
        const bool ib = tc_ref_is_bytes(arr_ref);
        int32_t maxlen = tc_ref_maxlen(vm, arr_ref);
        result = 1;
        for (int32_t i = 0; prefix[i] != 0; i++) {
          int32_t c = (i < maxlen) ? tc_chr_get(arr, ib, i) : 0;
          if (i >= maxlen || c == 0 || c != (int32_t)(uint8_t)prefix[i]) {
            result = 0;
            break;
          }
        }
    ...
    case SYS_STR_ENDS_CONST: {
      // strEndsWith(arr, "suffix") -> 1 if arr ends with suffix, else 0
      int32_t ci = TC_POP(vm);
      int32_t arr_ref = TC_POP(vm);
      int32_t *arr = tc_resolve_ref(vm, arr_ref);
      int32_t result = 0;
      if (arr && ci >= 0 && ci < vm->const_count && vm->constants[ci].type == 1) {
        const char *suffix = vm->constants[ci].str.ptr;
        const bool ib = tc_ref_is_bytes(arr_ref);
        int32_t maxlen = tc_ref_maxlen(vm, arr_ref);
        int32_t alen = tc_str_len_b(arr, ib, maxlen);
        int32_t slen = strlen(suffix);
        if (slen <= alen) {
          int32_t off = alen - slen;
          result = 1;
          for (int32_t i = 0; i < slen; i++) {
            if (tc_chr_get(arr, ib, off + i) != (int32_t)(uint8_t)suffix[i]) { result = 0; break; }
          }
        }
      }
      TC_PUSH(vm, result);
    ...
    case SYS_STR_CONTAINS_CONST: {
      // strContains(arr, "substr") -> 1 if substr appears anywhere in arr, else 0
      int32_t ci = TC_POP(vm);
      int32_t arr_ref = TC_POP(vm);
      int32_t *arr = tc_resolve_ref(vm, arr_ref);
      int32_t result = 0;
      if (arr && ci >= 0 && ci < vm->const_count && vm->constants[ci].type == 1) {
        ...
        int32_t nlen = strlen(needle);
        if (nlen > 0 && nlen <= alen) {
          for (int32_t i = 0; i <= alen - nlen; i++) {
            bool match = true;
            for (int32_t j = 0; j < nlen; j++) {
              if (tc_chr_get(arr, ib, i + j) != (int32_t)(uint8_t)needle[j]) { match = false; break; }
            }
    ...
    case SYS_STR_TO_UPPER: {
      // strToUpper(arr) — in-place ASCII A-Z conversion. UTF-8 multi-byte
      // characters above 0x7F are passed through unchanged.
      int32_t arr_ref = TC_POP(vm);
      int32_t *arr = tc_resolve_ref(vm, arr_ref);
      if (arr) {
        const bool ib = tc_ref_is_bytes(arr_ref);
        ...
        for (int32_t i = 0; i < maxlen; i++) {
          int32_t c = tc_chr_get(arr, ib, i);
          if (c == 0) break;
          if (c >= 'a' && c <= 'z') tc_chr_put(arr, ib, i, c - 32);
        }
      }
      break;
    }
    case SYS_STR_TO_LOWER: {
      // strToLower(arr) — in-place ASCII a-z conversion. UTF-8 multi-byte
      // characters above 0x7F are passed through unchanged.
      int32_t arr_ref = TC_POP(vm);
      int32_t *arr = tc_resolve_ref(vm, arr_ref);
      if (arr) {
    ...
    case SYS_STR_TRIM: {
      // strTrim(arr) — in-place strip leading + trailing whitespace
      // (' ', '\t', '\n', '\r'). Returns new length. Buffer is shifted
      // so arr[0] is the first non-whitespace byte.
      int32_t arr_ref = TC_POP(vm);
      int32_t *arr = tc_resolve_ref(vm, arr_ref);
      int32_t newlen = 0;
      if (arr) {
        ...
        int32_t s = 0;
        int32_t c;
        while (s < curlen && ((c = tc_chr_get(arr, ib, s)) == ' ' || c == '\t' || c == '\n' || c == '\r')) s++;
        // Find last non-whitespace
        int32_t e = curlen;
        while (e > s && ((c = tc_chr_get(arr, ib, e-1)) == ' ' || c == '\t' || c == '\n' || c == '\r')) e--;
        newlen = e - s;
        // Shift down
        if (s > 0) {
          for (int32_t i = 0; i < newlen; i++) tc_chr_put(arr, ib, i, tc_chr_get(arr, ib, s + i));
        }
        // Null-terminate
        if (newlen < maxlen) tc_chr_put(arr, ib, newlen, 0);
      }
      TC_PUSH(vm, newlen);
      break;
    }
    ...
    // ── Generic JSON parsing on an arbitrary script buffer (JsonParser-backed) ──
    // Unlike sensorGet (this device's own sensor JSON only), these parse ANY
    // char[] the script holds — e.g. an httpGet response from a remote device.
    // Path is '#'-separated like sensorGet ("StatusSNS#SHT3X#Temperature").
    case SYS_JSON_NUM: {                 // jsonNum(src, path) -> float
      ...
      JsonParserObject obj = parser.getRootObject();
      char *seg = jpath; char *next; bool valid = true;
      while (valid && seg && *seg) {
        next = strchr(seg, '#'); if (next) { *next = 0; next++; }
        JsonParserToken tok = obj[seg];
        if (!tok.isValid()) { valid = false; break; }
        if (next && *next) { obj = tok.getObject(); if (!obj.isValid()) { valid = false; break; } }
        else { fval = tok.getFloat(); }
        seg = next;
      }
      uint32_t fb; memcpy(&fb, &fval, 4);
      TC_PUSH(vm, (int32_t)fb);
      break;
    }
    case SYS_JSON_STR: {                 // jsonStr(src, path, dst) -> length (-1 absent)
      int32_t dst_ref  = TC_POP(vm);
      int32_t path_ref = TC_POP(vm);
      int32_t src_ref  = TC_POP(vm);
      char jpath[64]; TC_BUF(jbuf, 640);
    ...
    case SYS_HTTP_GET: {
      ...
      // valid HTTP 200 — always for a slow ESP8266 peer (chunks arrive in many
            ...
            if (gsize >= 0) {                        // identity body
              ...
              while (!done && len < cap - 1) {
                ...
                while (si < 11) {
                  ...
                  else if (millis() - t0 > TC_HTTP_IDLE_MS) break;
                  else delay(1);
                }
                if (!gotline) break;
                sz[si] = 0;
                long csz = strtol(sz, NULL, 16);
                ...
                int tr = 0; uint32_t tt = millis();    // trailing CRLF after chunk
                while (tr < 2) {
                  if (st->available()) { st->read(); tr++; tt = millis(); }
                  else if (!st->connected() && !st->available()) break;
                  else if (millis() - tt > TC_HTTP_IDLE_MS) break;
                  else delay(1);
                }
              }
            }
      ...
      // have moved while the mutex was released)
      {
        ...
        if (buf) {
          // ⚠️ Byte-aware, and it has to be: tc_ref_maxlen() already answered in
          // BYTES for a packed byte[] (1024 for `byte resp[1024]`), while the
          // copy below wrote int32 SLOTS. Handing httpGet a byte[] therefore
          // wrote four times its size and trampled the neighbouring heap
          // arrays -- silently, and only for a response long enough to reach
          // past the buffer. tc_chr_put() hides the packing, exactly as the
          // sprintf/strcat family does since 1.6.58.
          const bool ist_bytes = tc_ref_is_bytes(b);
          int32_t cap2 = tc_ref_maxlen(vm, b);
          int n = (result > 0 && lbuf) ? result : 0;
          if (n > cap2 - 1) n = cap2 - 1;
          for (int i = 0; i < n; i++) tc_chr_put(buf, ist_bytes, i, (int32_t)lbuf[i]);
          if (cap2 > 0) tc_chr_put(buf, ist_bytes, n, 0);
    ...
    case SYS_HTTP_POST: {
      ...
      if (httpCode > 0) {
        ...
        if (buf) {
          // Byte-aware, same reason as httpGet above: tc_ref_maxlen() counts
          // BYTES for a packed byte[] while a bare buf[i] writes int32 slots.
          const bool ist_bytes = tc_ref_is_bytes(respRef);
          int32_t maxLen = tc_ref_maxlen(vm, respRef);
          int len = payload.length();
          if (len > maxLen - 1) len = maxLen - 1;
          for (int i = 0; i < len; i++) tc_chr_put(buf, ist_bytes, i, (int32_t)(uint8_t)payload[i]);
          tc_chr_put(buf, ist_bytes, len, 0);
          result = len;
        }
      } else {
        result = httpCode;
    ...
    case SYS_WEB_BUTTON: {
      ...
      if (!label) label = "?";
      // Optional "Idle|Active" label: text after '|' is the brief click-
      // confirmation shown on the button for ~1.2 s, then it reverts.
      // No '|' -> generic checkmark feedback. No ON/OFF state suffix.
      const char *bar = strchr(label, '|');
      char idle[80]; char act[48];
      if (bar) {
        size_t n = (size_t)(bar - label);
        if (n >= sizeof(idle)) n = sizeof(idle) - 1;
        memcpy(idle, label, n); idle[n] = '\0';
        snprintf(act, sizeof(act), "%s", bar + 1);
      } else {
        snprintf(idle, sizeof(idle), "%s", label);
        act[0] = '\0';
      }
      int32_t nval = val ? 0 : 1;
      WSContentSend_P(PSTR("<div><button onclick='tcbtn(this,%d,%d)' data-a=\"%s\" style='width:100%%'>%s</button></div>"),
                      nval, idx, act, idle);
      break;
    }
    case SYS_WEB_TOGGLE: {
      ...
      const char *label = tc_get_const_str(vm, ci);
      if (!label) label = "?";
      // Optional "On text|Off text" label (emojis allowed): the part
      // before '|' is shown when the var != 0 (green), the part after
      // when 0 (grey). No '|' -> same text both states, colour only.
      const char *bar = strchr(label, '|');
      char shown[96];
      if (bar) {
        if (val) {
          ...
          memcpy(shown, label, n); shown[n] = '\0';
        } else {
          snprintf(shown, sizeof(shown), "%s", bar + 1);
        }
      } else {
        snprintf(shown, sizeof(shown), "%s", label);
      }
      int32_t nval = val ? 0 : 1;
      // Latching toggle: green when ON, grey when OFF.
      WSContentSend_P(PSTR("<div><button onclick='seva(%d,%d)' style='width:100%%;background:%s;color:#fff'>%s</button></div>"),
                      nval, idx, val ? "#0a0" : "#888", shown);
      break;
    }
    case SYS_WEB_SLIDER: {
      int32_t ci = TC_POP(vm);   // label const idx
    ...
    case SYS_WEB_BUTTON_V: {       // webButtonV(var, labelbuf) — like webButton, runtime label
      ...
      int32_t val = p ? *p : 0;
      char label[80];
      tc_ref_to_cstr(vm, lref, label, sizeof(label));
      const char *bar = strchr(label, '|');     // optional "Idle|clicked" feedback label
      char idle[80]; char act[48];
      if (bar) {
        size_t n = (size_t)(bar - label);
        if (n >= sizeof(idle)) n = sizeof(idle) - 1;
    ...
    case SYS_WEB_SLIDER_V: {       // webSliderV(var, min, max, labelbuf) — runtime label
      int32_t lref = TC_POP(vm);
      int32_t vmax = TC_POP(vm);
      int32_t vmin = TC_POP(vm);
      int32_t gref = TC_POP(vm);
      uint16_t idx = tc_widget_id(gref);
    ...
    case SYS_FAST_MUX: {           // fastMux(flag, time, buf, len) -> int (classic ESP32 / S3 + USE_TINYC_FAST_MUX)
      int32_t len  = TC_POP(vm);
      int32_t bref = TC_POP(vm);
      int32_t tval = TC_POP(vm);
      int32_t flag = TC_POP(vm);
#ifdef TC_FAST_MUX_ACTIVE
      int32_t *buf = tc_resolve_ref(vm, bref);
      int32_t maxl = tc_ref_maxlen(vm, bref);
      if (len < 0) { len = 0; }
      if (buf && len > maxl) { len = maxl; }      // never read past the array
      TC_PUSH(vm, tc_fast_mux((uint32_t)flag, (uint32_t)tval, buf, (uint32_t)len));
#else
      (void)len; (void)bref; (void)tval; (void)flag;
      TC_PUSH(vm, -1);                            // not built in on this target
#endif
      break;
    }
    ...
    // ⚠️ The ONLY widget that writes an ARRAY, and the reason it hands the
    // JavaScript the whole `gref` on top of the widget id: a char[]/byte[] over
    // HEAP_THRESHOLD (16) elements is heap-allocated, so tc_widget_id()'s low 12
    // bits are a heap HANDLE, not a global index -- and the ?sv= write-back used
    // to store the typed text into globals[handle] regardless. Handle 0 meant
    // globals[0]. Every webText on a buffer larger than 16 silently dropped what
    // the user typed and overwrote the first scalars of the script instead
    // (webui_demo/webcall_demo/multipage_demo's `char devname[32]`, lcd_i2c's
    // custom lines); only ip_in[16] in matter_bridge_ui, exactly at the
    // threshold, ever worked. Display was never affected -- that path reads
    // through tc_ref_to_cstr() below, which resolves heap refs correctly, so the
    // field showed the right text and simply refused to keep a new one.
    // With the ref in hand the write-back can just call tc_cstr_to_ref(), the
    // VM's own writer, which knows heap vs global AND the element packing.
    // (gemu 2026-08-26)
    case SYS_WEB_TEXT: {
      int32_t ci = TC_POP(vm);      // label const idx
      ...
      WSContentSend_P(PSTR("<div><label><b>%s</b> "
                           ...
                           "onchange='siva(value,%d,%d)'>"
                           "</label></div>"),
                      label, tbuf, maxlen - 1, idx, (int)gref);
      break;
    }
    case SYS_WEB_NUMBER: {
      int32_t ci = TC_POP(vm);
    ...
    case SYS_WEB_CHART_Q: {
      // WebChartQ(scale, offset) — the NEXT WebChart decodes each sample as
      // value = raw * scale + offset. Its reason for existing is the packed
      // byte[]: a raw 0..255 is not a temperature, and without a scale the
      // memory saving would only be reachable by charts of dimensionless
      // numbers. Applies to any array type, so it doubles as a unit conversion
      // (W -> kW) on an existing float[].
      float offset = i2f(TC_POP(vm));
      float scale  = i2f(TC_POP(vm));
      // A zero scale flattens every sample onto `offset` — always a mistake,
      // and a silent one: the chart draws a straight line at the wrong height
      // instead of failing. Refuse it and stay at identity.
      if (scale == 0.0f) {
        AddLog(LOG_LEVEL_INFO, PSTR("TCC: WebChartQ scale 0 ignored — chart would collapse to a flat line"));
        break;
      }
      tc_chart_q_scale  = scale;
      tc_chart_q_offset = offset;
      break;
    }
    ...
    case SYS_WEB_CHART: {
      ...
      // Take the WebChartQ pair and clear it HERE, before any of the early
      // breaks below can skip past. One WebChart is one series, and two byte[]
      // series in the same chart normally want different scales — so the
      // setting is one-shot, unlike WebChartSize, whose stickiness across
      // slots is the standing complaint about chart layout. Identity is left
      // as a straight copy so an unquantised float[] stays bit-for-bit what it
      // always was.
      const float q_scale  = tc_chart_q_scale;
      const float q_offset = tc_chart_q_offset;
      const bool  q_active = (q_scale != 1.0f) || (q_offset != 0.0f);
      tc_chart_q_scale  = 1.0f;
      tc_chart_q_offset = 0.0f;
      ...
      if (!tc_chart_lib_sent) {
        WSContentSend_P(PSTR(
          ...
          // _tcAy: the same series, sent as a bare y-list plus the x sequence
          // that generated it. Every x WebChart ever emitted was
          // -(count-1-i)*interval + timebase, i.e. an arithmetic sequence, so
          // repeating it per point cost about 8 of the ~14 bytes each point
          // took on the wire. Rebuilding it in the browser costs one loop and
          // hands the rest of the file the identical [x,y] shape it already
          // consumes -- nothing downstream of the push knows the difference.
          "function _tcAy(ci,lbl,clr,x0,st,y,mn,mx,u){var d=[],i=0;for(;i<y.length;i++)d.push([x0+i*st,y[i]]);_tcA(ci,lbl,clr,d,mn,mx,u);}"
            ...
            "for(var i=0;i<_tcC.length;i++){"
              ...
              "}else{"                                                                     // charts
                ...
                "if(c.s.length>0)for(var k=0;k<c.s[0].d.length;k++){"
                  "var m=c.s[0].d[k][0];"
                  "var xl;"
                  // Column-chart x labels. Chosen by the SPAN the series covers,
                  // not by how many points it has -- the old count rule read
                  // "more than 7 points" as "more than a week" and stamped the
                  // DATE on every column. On a 24 h rain chart (288 columns,
                  // 5 min apart) that is the same day printed 288 times, and
                  // the one thing the reader needs -- the hour it rained -- is
                  // nowhere on the axis (gemu, .98, 2026-08-25).
                  // Span is (points-1) x step, in minutes, straight from the
                  // x values: <= ~36 h -> HH:MM, <= ~8 days -> weekday, else
                  // the date. Every existing chart keeps its old label: 7 daily
                  // bars still span a week (weekday), 31 daily bars a month
                  // (date), 12 monthly bars a year (date).
                  "if(isCol){"
                    "var dd=c.s[0].d,sp=dd.length>1?Math.abs(dd[dd.length-1][0]-dd[0][0]):0;"
                    "if(tp==115&&dd.length>7){xl=''+k;}"
                    "else{var td=new Date(N.getTime()+m*60000);"
                      "if(dd.length==1){xl='';}"
                      "else if(sp<=2160){xl=('0'+td.getHours()).slice(-2)+':'+('0'+td.getMinutes()).slice(-2);}"
                      "else if(sp<=11520){xl=['So','Mo','Di','Mi','Do','Fr','Sa'][td.getDay()];}"
                      "else{xl=('0'+td.getDate()).slice(-2)+'.'+('0'+(td.getMonth()+1)).slice(-2)+'.';}"
                    "}"
                  "}else{xl=new Date(N.getTime()+m*60000);}"
                  "var r=[xl];"
                  "for(var j=0;j<c.s.length;j++)r.push(c.s[j].d[k][1]);"
                  "rows.push(r);}"
                "dt.addRows(rows);"
                ...
                // Locale-aware decimal separator on numeric columns (so tooltips
                // and any formatted axis ticks show `1,5` on a de_DE build, `1.5`
                // on en_GB, etc.). Driven by Tasmota's compile-time
                // D_DECIMAL_SEPARATOR — same mechanism Tasmota uses for its own
                // sensor body. JS literals in chart data stay '.' (correct).
                "if('" D_DECIMAL_SEPARATOR "'!='.'){"
                  "var nf=new google.visualization.NumberFormat({decimalSymbol:'" D_DECIMAL_SEPARATOR "'});"
                  "for(var j=1;j<=c.s.length;j++)nf.format(dt,j);"
                "}"
      ...
      // 4) Resolve the sample array and emit it as one _tcAy() call.
      // A PACKED byte[] carries one sample per byte instead of one per int32
      // slot — a quarter of the RAM for the ring buffer, which is what the
      // chart actually costs on a small device. The flag rides on the ref, so
      // the packing is invisible to the script: it declares `byte h[288]`
      // instead of `float h[288]` and adds a WebChartQ() to say what a raw
      // 0..255 means. tc_resolve_ref strips the flag (the byte view sits on
      // top of the same storage), and tc_ref_maxlen already answers in BYTES
      // for a flagged ref — which for a byte[] is exactly the element count,
      // so the clamp below needs no special case.
      // A packed int16[] is the middle option: half the RAM of a float[] but,
      // unlike a byte[], at the FULL resolution of the reading, so it needs no
      // WebChartQ to be meaningful -- hundredths of a kelvin, a raw ADC word or
      // a Modbus register go in as they are. Signedness comes off the ref: an
      // uint16[] carries its own kind precisely so 60000 does not draw as -5536.
      const int elem_kind = tc_ref_elem_kind(arr_ref);
      int32_t *arr = tc_resolve_ref(vm, arr_ref);
      if (!arr) break;
      int32_t arr_len = tc_ref_elem_count(vm, arr_ref);   // in SAMPLES, any width
      ...
      // The x values are an arithmetic sequence — first sample at
      // -(count-1)*interval, one `interval` step per sample — so they are sent
      // ONCE as (x0, step) and rebuilt in _tcAy instead of being repeated in
      // full next to every y. On a 1441-point series that is the difference
      // between ~20 KB and ~8 KB of response body, and the body is what has to
      // squeeze through the TLS send path on a C3.
      int32_t x0 = -((count - 1) * interval) + tc_chart_time_base;
      WSContentSend_P(PSTR("<script>_tcAy(%d,'%s','%s',%d,%d,["),
        chart_id, series_name, cbuf, x0, interval);

      // Unwind ring buffer: oldest entry first
      ...
      for (int32_t i = 0; i < count; i++) {
        ...
        if (idx < 0) idx += count;
        float fval;
        if (elem_kind == TC_ELEM_BYTE) {
          fval = (float)tc_chr_get(arr, true, idx);   // 0..255, meaning comes from WebChartQ
        } else if (elem_kind == TC_ELEM_I16) {
          fval = (float)((const int16_t *)arr)[idx];
        } else if (elem_kind == TC_ELEM_U16) {
          fval = (float)((const uint16_t *)arr)[idx];
        } else {
          memcpy(&fval, &arr[idx], sizeof(float));
        }
        if (q_active) fval = fval * q_scale + q_offset;
        char vbuf[16];
        dtostrf(fval, 1, decimals, vbuf);
        WSContentSend_P(PSTR("%s%s"), (i > 0) ? "," : "", vbuf);
    ...
    case SYS_SORT_ARRAY: {
      ...
      int32_t maxLen = tc_ref_elem_count(vm, arr_ref);    // in ELEMENTS, any width
      if (count > maxLen) count = maxLen;
      // A packed array sorts in its OWN element width. Doing it through the
      // int32 view would shuffle whole slots -- four byte samples or two 16-bit
      // ones at a time -- which reorders nothing the script asked for. Floats
      // do not exist in a packed array, so these branches are integer-only.
      const int sort_kind = tc_ref_elem_kind(arr_ref);
      if (sort_kind != TC_ELEM_I32) {
        const bool absteigend = (flags & 2) != 0;
        for (int32_t i = 1; i < count; i++) {
          int32_t key;
          if      (sort_kind == TC_ELEM_BYTE) key = ((uint8_t  *)arr)[i];
          else if (sort_kind == TC_ELEM_I16)  key = ((int16_t  *)arr)[i];
          else                                key = ((uint16_t *)arr)[i];
          int32_t j = i - 1;
          while (j >= 0) {
            int32_t vj;
            if      (sort_kind == TC_ELEM_BYTE) vj = ((uint8_t  *)arr)[j];
            else if (sort_kind == TC_ELEM_I16)  vj = ((int16_t  *)arr)[j];
            else                                vj = ((uint16_t *)arr)[j];
            if (!(absteigend ? (vj < key) : (vj > key))) break;
            if      (sort_kind == TC_ELEM_BYTE) ((uint8_t  *)arr)[j + 1] = (uint8_t)vj;
            else                                ((uint16_t *)arr)[j + 1] = (uint16_t)vj;
            j--;
          }
          if      (sort_kind == TC_ELEM_BYTE) ((uint8_t  *)arr)[j + 1] = (uint8_t)key;
          else                                ((uint16_t *)arr)[j + 1] = (uint16_t)key;
        }
        break;
      }
    ...
    case SYS_BASE64_ENC: {
      ...
      if (in_len > 0) {
        ...
        TC_UBUF(raw, 512);
        // input accepts char[] OR byte[] via tc_ref_bytes.
        int32_t rn = 0;
        uint8_t *rp = tc_ref_bytes(vm, in_ref, in_len, raw, sizeof(raw), &rn);
        if (rp && rn > 0) {
          in_len = rn;
          TC_UBUF(ob, 704); size_t olen = 0;
          if (mbedtls_base64_encode(ob, sizeof(ob), &olen, rp, in_len) == 0) {
            int32_t *obuf = tc_resolve_ref(vm, out_ref);
            int32_t omax = tc_ref_maxlen(vm, out_ref);
            const bool out_is_bytes = tc_ref_is_bytes(out_ref);
            if (obuf && omax > 0) {
              int i;
              for (i = 0; i < (int)olen && i < omax - 1; i++) {
                if (out_is_bytes) ((uint8_t*)obuf)[i] = ob[i];
                else              obuf[i] = ob[i];
              }
              if (out_is_bytes) ((uint8_t*)obuf)[i] = 0;
              else              obuf[i] = 0;
              ret = i;
            }
          }
    ...
    case SYS_TCP_READ_ARR: {  // wsra(arr)
      ...
      if (_tc) {
        ...
        if (base) {
          ...
          for (uint16_t i = 0; i < slen; i++) {
            int b = _tc->read();
            if (is_bytes) ((uint8_t*)base)[i] = (uint8_t)b;
            else          base[i] = b;
          }
          count = slen;
        }
    ...
    case SYS_TCP_WRITE_ARR: {  // wswa(arr, num, type)
      ...
      if (_tc) {
        int32_t *base = tc_resolve_ref(vm, ref);
        if (base) {
          const bool is_bytes = tc_ref_is_bytes(ref);   // byte[] source: element i is one byte
          uint8_t *abf = (uint8_t*)malloc(num * 4);
          if (abf) {
            uint8_t *p = abf;
            uint16_t dlen = 0;
            for (int32_t i = 0; i < num; i++) {
              int32_t val = is_bytes ? (int32_t)((uint8_t*)base)[i] : base[i];
              switch (type) {
                case 0:  // uint8
                  *p++ = (uint8_t)val;
                  dlen++;
      ...
      if (TasmotaGlobal.global_state.network_down) {
        ...
        if (!ip) {
          ...
          if (_cok) {
            ...
            Tinyc->tcp_cli_reason[Tinyc->tcp_cli_slot] = 4;  // NETWORK
    ...
    case SYS_TCP_CONNECTED: {  // tcpConnected() -> int
      // Detect external-disconnect transitions and update reason.
      // If reason was CONNECTED (1) but socket is down, peer dropped us.
      // We can't easily distinguish RST/FIN from kernel timeout — flag as
      // PEER_CLOSED (2). User-initiated disconnects already set reason=5
      // before we get here.
      bool conn = TC_TCP_OUT_CLIENT()->connected();
      uint8_t r = Tinyc->tcp_cli_reason[Tinyc->tcp_cli_slot];
      if (!conn && r == 1) {
    ...
    case SYS_TCP_KEEPALIVE: {  // tcpKeepalive(idle_sec, intvl_sec, count) -> int 1=ok 0=err
      // Set SO_KEEPALIVE + TCP_KEEPIDLE/INTVL/CNT on the selected slot.
      // The slot must be currently connected for setsockopt to take.
      // Typical SMA Tripower setting: tcpKeepalive(30, 10, 3) — after 30 s
      // idle, send up to 3 probes spaced 10 s apart before declaring dead.
      int32_t cnt   = TC_POP(vm);
      int32_t intvl = TC_POP(vm);
      int32_t idle  = TC_POP(vm);
      WiFiClient *_oc = TC_TCP_OUT_CLIENT();
      int rc = 0;
      if (_oc->connected()) {
#ifdef ESP32
        // Arduino-ESP32's NetworkClient (parent of WiFiClient) provides
        // setSocketOption(level, option, value, valueLen). Use raw IPPROTO_TCP
        // constants rather than the higher-level setNoDelay/etc. for portability
    ...
    case SYS_TCP_NODELAY: {  // tcpNoDelay(on) -> void
      // Toggle Nagle's algorithm on the selected slot. Nagle is OFF by default
      // (set by every successful connect) — call tcpNoDelay(0) to RE-enable
      // Nagle if you really want batched small writes.
      int32_t on = TC_POP(vm);
      WiFiClient *_oc = TC_TCP_OUT_CLIENT();
      if (_oc->connected()) {
        _oc->setNoDelay(on != 0);
    ...
    case SYS_TCP_DISCONNECT_REASON: {  // tcpDisconnectReason() -> int 0..5
      // Read last-known disconnect reason for the selected slot.
      // Side-effect-aware: if connected() shows the link is down but the
      // tracked reason is still CONNECTED, transition to PEER_CLOSED here too
      // (mirrors SYS_TCP_CONNECTED). Lets a script poll this without first
      // having to call tcpConnected().
      bool conn = TC_TCP_OUT_CLIENT()->connected();
      uint8_t r = Tinyc->tcp_cli_reason[Tinyc->tcp_cli_slot];
      if (!conn && r == 1) {
        Tinyc->tcp_cli_reason[Tinyc->tcp_cli_slot] = 2;  // PEER_CLOSED
      ...
      TC_PUSH(vm, (int32_t)r);
      break;
    }
    case SYS_TCP_TRANSACT: {  // tcpTransact(req, req_len, resp, resp_max, timeout_ms) -> int
      // Atomic write-and-await-reply for request/response protocols
      // (Modbus-TCP, etc.). Folds the write-poll-read trio into one
      // syscall so the script doesn't ship its own race-prone busy-loop.
      // Wait granularity is 1 FreeRTOS tick (≈1 ms); other tasks run
      // freely during the gaps between availability checks, but THIS
      // slot's vm_mutex is held throughout — keep timeouts reasonable
      // (≤200 ms is the recommended ceiling).
      int32_t timeout_ms = TC_POP(vm);
      int32_t resp_max   = TC_POP(vm);
      int32_t resp_ref   = TC_POP(vm);
      int32_t req_len    = TC_POP(vm);
      int32_t req_ref    = TC_POP(vm);
      ...
      int32_t *resp_base = tc_resolve_ref(vm, resp_ref);
      int32_t *req_base  = (req_len > 0) ? tc_resolve_ref(vm, req_ref) : nullptr;
      if (!resp_base || (req_len > 0 && !req_base)) {
        TC_PUSH(vm, -3);
        break;
      }

      // Marshal req char[] (1 byte per int32 slot) into a contiguous
      // wire buffer; small writes go on the stack, larger ones (>256 B
      // — rare for Modbus, max FC16 is 13+246=259 B) malloc briefly.
      int32_t result = -1;
      if (req_len > 0) {
        uint8_t  stack_buf[256];
        uint8_t *write_buf = (req_len <= (int32_t)sizeof(stack_buf))
                             ? stack_buf : (uint8_t*)malloc((size_t)req_len);
        if (!write_buf) {
          TC_PUSH(vm, -3);
          break;
        }
        for (int32_t i = 0; i < req_len; i++) {
          write_buf[i] = (uint8_t)(req_base[i] & 0xFF);
        }
        size_t wrote = _oc->write(write_buf, (size_t)req_len);
        if (write_buf != stack_buf) free(write_buf);
        if ((int32_t)wrote != req_len) {
          // Short write means the socket buffer's full or the peer's
          // already gone — treat as disconnect so the script reconnects
          // rather than retrying with a partially-sent request.
          if (Tinyc->tcp_cli_reason[Tinyc->tcp_cli_slot] == 1) {
            Tinyc->tcp_cli_reason[Tinyc->tcp_cli_slot] = 2;  // PEER_CLOSED
          }
          TC_PUSH(vm, -2);
          break;
      ...
      // Wait for any response within the deadline. As soon as `available()`
      // reports >0, drain up to resp_max bytes and return — for Modbus and
      // similar one-shot protocols the whole reply lands in one segment so
      // first non-zero check has it all. vTaskDelay(1) yields the CPU each
      // cycle so other FreeRTOS tasks aren't starved.
      uint32_t deadline = millis() + (uint32_t)timeout_ms;
      while (1) {
        if (!_oc->connected()) {
          if (Tinyc->tcp_cli_reason[Tinyc->tcp_cli_slot] == 1) {
            Tinyc->tcp_cli_reason[Tinyc->tcp_cli_slot] = 2;
          }
          result = -2;
          break;
        }
        int avail = _oc->available();
        if (avail > 0) {
          int to_read = (avail < resp_max) ? avail : resp_max;
          for (int i = 0; i < to_read; i++) {
            int b = _oc->read();
            resp_base[i] = (b < 0) ? 0 : (int32_t)(b & 0xFF);
          }
          result = to_read;
          break;
        }
        if ((int32_t)(deadline - millis()) <= 0) {
          // True timeout — no data within the window. Update the
          // disconnect-reason field so the watchdog can distinguish a
          // dead-but-still-connected peer from a hard disconnect.
          Tinyc->tcp_cli_reason[Tinyc->tcp_cli_slot] = 3;  // TIMEOUT
          result = -1;
          break;
        }
#ifdef ESP32
    ...
    case SYS_TWAI_RECV: {  // (meta_arr, data_buf, max_bytes) -> int bytes_read | -1
      ...
      int32_t copy_n = (msg.data_length_code < (uint32_t)max_bytes)
                        ...
                        : max_bytes;
      // data buffer accepts char[] OR byte[] (the CAN payload is bytes); the
      // meta array stays int32 (id, ext, dlc are values, not bytes).
      if (tc_ref_is_bytes(buf_ref)) {
        for (int32_t k = 0; k < copy_n; k++) ((uint8_t*)buf_p)[k] = msg.data[k];
      } else {
        for (int32_t k = 0; k < copy_n; k++) buf_p[k] = (int32_t)msg.data[k];
      }
      TC_PUSH(vm, copy_n);
      break;
    }
    case SYS_TWAI_SEND: {  // (id, ext, dlc, data_buf) -> int 1=ok 0=err
      ...
      // data buffer accepts char[] OR byte[] (CAN payload is bytes).
      if (dlc > 0 && tc_ref_is_bytes(buf_ref)) {
        for (int32_t k = 0; k < dlc; k++) msg.data[k] = ((uint8_t*)buf_p)[k];
      } else {
        for (int32_t k = 0; k < dlc; k++) msg.data[k] = (uint8_t)(buf_p[k] & 0xFF);
      }
    ...
    case SYS_LVGL_CHART_UPD:    { int32_t m = TC_POP(vm); int32_t h = TC_POP(vm); tc_lv_chart_update_mode(h, m); break; }
    ...
    case SYS_LVGL_CHART_UPD: TC_POP(vm); TC_POP(vm); break;
    ...
    // ⚠️ MQTT can be switched off on the device: `SetOption3` (default MQTT_USE
    // from my_user_config.h). With it OFF, MqttPublishPayload still runs but
    // `published` stays false — the message does NOT go out, and all of Xdrv02
    // stops answering MQTT commands (which is why such a device reports
    // "MqttHost: Unknown"). A script should be able to tell that apart instead
    // of sending into the void: -2 = MQTT is disabled (gemu 2026-08-18, set
    // exactly that way on his C3).
    case SYS_MQTT_PUBLISH_REF: {  // mqttPublish(topic_ref, payload_ref, level)
      int32_t lvl  = TC_POP(vm);
      int32_t pref = TC_POP(vm);
      int32_t tref = TC_POP(vm);
      // Both refs go through tc_ref_to_cstr, which takes a runtime char[] as
      // well as a literal passed down as a const-ref -- so the same call works
      // either way round.
      char topic[TC_MQTT_TOPIC_MAX];
      char payload[TC_MQTT_PAYLOAD_MAX];
      ...
      if (!Settings->flag.mqtt_enabled) { TC_PUSH(vm, -2); break; }   // SetOption3 off
    ...
    case SYS_MQTT_PUBLISH_TO: {  // mqttPublish("topic", "payload") -> int 0=ok or -1
      ...
      if (!Settings->flag.mqtt_enabled) {
        TC_PUSH(vm, -2);                                 // SetOption3 off — see above
    ...
    case SYS_SHARE_SET_INT: {
      ...
      const char *key = tc_get_const_str(vm, ki);
      if (!key) {
        AddLog(LOG_LEVEL_INFO, PSTR("TCC: shareSetInt: null key (const-pool index %d invalid)"), (int)ki);
        break;
      }
      tc_share_lock();
      int idx = tc_share_find_or_alloc(key);
      if (idx >= 0) {
        tc_share_table[idx].type = TC_SHARE_TYPE_INT;
        tc_share_table[idx].v.i = val;
      } else {
        int used = tc_share_count_used();
        tc_share_unlock();
        AddLog(LOG_LEVEL_INFO, PSTR("TCC: shareSetInt(\"%s\")=%d failed — table full (%d/%d). Raise TC_SHARE_MAX in user_config_override.h."),
               key, (int)val, used, (int)TC_SHARE_MAX);
        break;
      }
      tc_share_unlock();
      break;
    ...
    case SYS_SHARE_GET_INT: {
      ...
      const char *key = tc_get_const_str(vm, ki);
      int32_t out = 0;
      // 1.6.23 — bounded try-lock (Andreas's .104 HTTP-hang fix). If a writer
      // is in the middle of a burst, drop the read with out=0 (same as missing
      // key) instead of blocking the HTTP task forever. Writers keep
      // portMAX_DELAY so values do get persisted.
      if (key && tc_share_try_lock_ms(50)) {
        int idx = tc_share_find(key);
        if (idx >= 0) {
    ...
    case SYS_SHARE_SET_FLT: {
      ...
      const char *key = tc_get_const_str(vm, ki);
      if (!key) {
        AddLog(LOG_LEVEL_INFO, PSTR("TCC: shareSetFloat: null key (const-pool index %d invalid)"), (int)ki);
        break;
      }
      tc_share_lock();
      int idx = tc_share_find_or_alloc(key);
    ...
    case SYS_SHARE_GET_FLT: {
      ...
      const char *key = tc_get_const_str(vm, ki);
      float out = 0.0f;
      // 1.6.23 — bounded try-lock (see SYS_SHARE_GET_INT note).
      if (key && tc_share_try_lock_ms(50)) {
        int idx = tc_share_find(key);
        if (idx >= 0) {
    ...
    case SYS_SHARE_SET_STR: {
      ...
      int32_t *src = tc_resolve_ref(vm, src_ref);
      if (!key) {
        AddLog(LOG_LEVEL_INFO, PSTR("TCC: shareSetStr: null key (const-pool index %d invalid)"), (int)ki);
        break;
      }
      if (!src) {
        AddLog(LOG_LEVEL_INFO, PSTR("TCC: shareSetStr(\"%s\"): null src buffer (ref %d invalid)"), key, (int)src_ref);
        break;
      }
      ...
      buf[si] = '\0';
      // Detect truncation: source ran past buf capacity. Only flag when
      // the source still has content at si — i.e. we stopped because of
      // buf size, not source NUL.
      bool truncated = (si == (int32_t)sizeof(buf) - 1) && (si < srcMax) && (src[si] != 0);
      if (truncated) {
        AddLog(LOG_LEVEL_INFO, PSTR("TCC: shareSetStr(\"%s\") source truncated at %d B (TC_SHARE_STR_LEN=%d). Raise it in user_config_override.h if you need longer strings."),
               key, (int)si, (int)TC_SHARE_STR_LEN);
      }
      tc_share_lock();
      int idx = tc_share_find_or_alloc(key);
      if (idx >= 0) {
        ...
        int used = tc_share_count_used();
        tc_share_unlock();
        AddLog(LOG_LEVEL_INFO, PSTR("TCC: shareSetStr(\"%s\")=\"%.32s%s\" failed — table full (%d/%d). Raise TC_SHARE_MAX in user_config_override.h."),
               key, buf, (si > 32) ? "..." : "", used, (int)TC_SHARE_MAX);
        break;
      }
      tc_share_unlock();
      break;
    ...
    case SYS_SHARE_GET_STR: {
      ...
      if (key && dst) {
        ...
        char buf[TC_SHARE_STR_LEN + 1];
        buf[0] = '\0';
        // 1.6.23 — bounded try-lock (see SYS_SHARE_GET_INT note). STR is the
        // only reader where the lock actually matters (multi-byte copy under
    ...
    case SYS_SHARE_DUMP: {
      // shareDump() — walk the entire share table and log each live entry
      // as `TCC: share[N] key="KEY" type=TYPE value=VALUE`. Returns the
      // count of live entries. Pure diagnostic: lets a script verify
      // cross-VM writes actually land (Andreas's .107 use case where Slot
      // 2 writes don't appear when read from Slot 0). One log line per
      // live entry plus a summary line, so heavy on serial output —
      // intended for one-shot diagnostic calls, not periodic polling.
      tc_share_lock();
      int32_t live = 0;
      for (int i = 0; i < TC_SHARE_MAX; i++) {
        if (tc_share_table[i].type == TC_SHARE_TYPE_NONE) continue;
        live = live + 1;
        const char *tname;
        switch (tc_share_table[i].type) {
          case TC_SHARE_TYPE_INT: tname = "INT"; break;
          case TC_SHARE_TYPE_FLT: tname = "FLT"; break;
          case TC_SHARE_TYPE_STR: tname = "STR"; break;
          default:                tname = "???"; break;
        }
        if (tc_share_table[i].type == TC_SHARE_TYPE_FLT) {
          char fbuf[24];
          dtostrfd(tc_share_table[i].v.f, 6, fbuf);
      ...
      tc_share_unlock();
      AddLog(LOG_LEVEL_INFO, PSTR("TCC: shareDump: %d/%d live entries"), (int)live, TC_SHARE_MAX);
      TC_PUSH(vm, live);
      break;
    }
#endif // TC_USE_SHARE
    case SYS_PERSIST_DUMP: {
      ...
      // (which resets ALL persist values to defaults). Mirrors
      // shareDump(): one-shot diagnostic, heavy on serial. Raw int32
      // words are emitted (restorable bit-exact regardless of int/float
      // — persist stores no per-slot type), chunked 16/line so long
      // arrays are never silently truncated.
      AddLog(LOG_LEVEL_INFO,
             PSTR("TCC: persistDump file=\"%s\" hash=0x%08X entries=%d"),
             vm->persist_file[0] ? vm->persist_file : "(none)",
             (unsigned)tc_persist_layout_hash(vm), (int)vm->persist_count);
      for (uint8_t k = 0; k < vm->persist_count; k++) {
        uint16_t idx = vm->persist[k].index;
        uint16_t cnt = vm->persist[k].count;
        char line[160];
        uint16_t off = 0;
        while (off < cnt) {
          int pos = 0;
          pos += snprintf(line + pos, sizeof(line) - pos,
                          "p[%u] i=%u n=%u @%u:", k, idx, cnt, off);
          uint16_t end = off + 16; if (end > cnt) end = cnt;
          for (uint16_t j = off; j < end && pos < (int)sizeof(line) - 14; j++) {
            int32_t w = (idx + j < TC_MAX_GLOBALS) ? vm->globals[idx + j] : 0;
            pos += snprintf(line + pos, sizeof(line) - pos,
                            "%s%ld", (j > off ? "," : ""), (long)w);
          }
          AddLog(LOG_LEVEL_INFO, PSTR("TCC: persist %s"), line);
          off = end;
        }
      }
      TC_PUSH(vm, (int32_t)vm->persist_count);
      break;
    }
    ...
    case SYS_AES_ECB: {
      ...
      // key and data accept char[] OR byte[]: tc_ref_bytes hands out a direct
      // pointer for byte[] and copies the low bytes into k[]/in[] for int32.
      uint8_t k[16], in[16], out[16];
      int32_t klen = 0, dlen = 0;
      uint8_t *kp = tc_ref_bytes(vm, key_ref,  16, k,  sizeof(k),  &klen);
      uint8_t *dp = tc_ref_bytes(vm, data_ref, 16, in, sizeof(in), &dlen);
      if (!kp || !dp || klen < 16 || dlen < 16) {
        TC_PUSH(vm, 0); break;
      }
      mbedtls_aes_context ctx;
      mbedtls_aes_init(&ctx);
      int rc = enc_flag ? mbedtls_aes_setkey_enc(&ctx, kp, 128)
                        : mbedtls_aes_setkey_dec(&ctx, kp, 128);
      if (rc == 0) {
      ...
      if (rc == 0) {
        tc_ref_put_bytes(vm, data_ref, out, 16);
        TC_PUSH(vm, 1);
      } else {
        TC_PUSH(vm, 0);
      }
    ...
    case SYS_AES_CBC: {
      ...
      // key/iv/data accept char[] OR byte[] via tc_ref_bytes: for byte[] `bufp`
      // points straight into the region (in-place CBC, no copy); for int32 the
      // low bytes are gathered into `buf` and written back afterwards.
      uint8_t k[16], iv[16];
      uint8_t *buf = (uint8_t*)special_malloc(len);
      if (!buf) { TC_PUSH(vm, 0); break; }
      int32_t klen = 0, ivlen = 0, dlen = 0;
      uint8_t *kp   = tc_ref_bytes(vm, key_ref,  16,  k,   sizeof(k),  &klen);
      uint8_t *ivp  = tc_ref_bytes(vm, iv_ref,   16,  iv,  sizeof(iv), &ivlen);
      uint8_t *bufp = tc_ref_bytes(vm, data_ref, len, buf, len,        &dlen);
      if (!kp || !ivp || !bufp || klen < 16 || ivlen < 16 || dlen < len) {
        free(buf); TC_PUSH(vm, 0); break;
      }
      mbedtls_aes_context ctx;
      mbedtls_aes_init(&ctx);
      int rc = enc_flag ? mbedtls_aes_setkey_enc(&ctx, kp, 128)
      ...
      mbedtls_aes_free(&ctx);
      if (rc == 0) {
        tc_ref_put_bytes(vm, data_ref, bufp, len);
        tc_ref_put_bytes(vm, iv_ref,   ivp,  16);   // updated IV
        TC_PUSH(vm, 1);
      } else {
        TC_PUSH(vm, 0);
      }
    ...
    case SYS_HMAC_SHA256: {
      ...
      // key/data accept char[] OR byte[] via tc_ref_bytes; the digest is
      // written back with tc_ref_put_bytes, so out_ref may be byte[] too.
      ...
      int32_t kn = 0, dn = 0;
      uint8_t *kp = tc_ref_bytes(vm, key_ref,  klen, kbuf, klen, &kn);
      uint8_t *dp = tc_ref_bytes(vm, data_ref, dlen, dbuf, dlen > 0 ? dlen : 1, &dn);
      if (!kp || (dlen > 0 && !dp)) { free(kbuf); free(dbuf); TC_PUSH(vm, 0); break; }
      const mbedtls_md_info_t *info = mbedtls_md_info_from_type(MBEDTLS_MD_SHA256);
      int rc = mbedtls_md_hmac(info, kp, klen, dp, dlen, out);
      free(kbuf); free(dbuf);
      if (rc == 0) {
        tc_ref_put_bytes(vm, out_ref, out, 32);
        TC_PUSH(vm, 1);
      } else {
        TC_PUSH(vm, 0);
      }
    ...
    case SYS_SHA256: {
      ...
      // data accepts char[] OR byte[]; the digest goes back via put_bytes.
      uint8_t *dbuf = (uint8_t*)special_malloc(dlen > 0 ? dlen : 1);
      uint8_t out[32];
      if (!dbuf) { TC_PUSH(vm, 0); break; }
      int32_t dn = 0;
      uint8_t *dp = tc_ref_bytes(vm, data_ref, dlen, dbuf, dlen > 0 ? dlen : 1, &dn);
      if (dlen > 0 && !dp) { free(dbuf); TC_PUSH(vm, 0); break; }
      // mbedtls_sha256(buf, len, out, is_sha224=0) — return 0 on success.
      int rc = mbedtls_sha256(dp, dlen, out, 0);
      free(dbuf);
      if (rc == 0) {
        tc_ref_put_bytes(vm, out_ref, out, 32);
        TC_PUSH(vm, 1);
    ...
    case SYS_MD5: {                 // md5(data_ref, dlen, out16_ref) -> 1 ok / 0 err
      ...
      uint8_t *dp = tc_ref_bytes(vm, data_ref, dlen, dbuf, dlen > 0 ? dlen : 1, &dn);
      if (dlen > 0 && !dp) { free(dbuf); TC_PUSH(vm, 0); break; }
      int rc = mbedtls_md(info, dp, dlen, out);
      free(dbuf);
      if (rc == 0) {
        tc_ref_put_bytes(vm, out_ref, out, 16);
        TC_PUSH(vm, 1);
      } else {
        TC_PUSH(vm, 0);
      }
    ...
    case SYS_HEX2BIN: {
      ...
      const bool out_is_bytes = tc_ref_is_bytes(out_ref);   // byte[] output packs 1:1
      ...
      if (tc_is_const_ref(hex_ref)) {
        ...
        // hex input accepts char[] OR byte[] via tc_ref_bytes.
        if (hex_len > 1024) hex_len = 1024;
        int32_t hn = 0;
        uint8_t *hp = tc_ref_bytes(vm, hex_ref, hex_len, (uint8_t*)src, 1024, &hn);
        if (!hp) { free(src); TC_PUSH(vm, -1); break; }
        if (hp != (uint8_t*)src) { memcpy(src, hp, hn); }   // byte[]: copy out of the region
        src_len = hn;
      }
      // Parse pairs of nibbles, skipping whitespace. Bail on malformed.
      int32_t out_cap = tc_ref_maxlen(vm, out_ref);   // byte[] -> bytes, else slots
      int32_t written = 0;
      int bad = 0;
      ...
      for (int i = 0; i < src_len; i++) {
        ...
        if (!have_hi) { hi = v; have_hi = 1; }
          ...
          if (out_is_bytes) ((uint8_t*)out_arr)[written] = b;
          else              out_arr[written] = (int32_t)b;
          written++;
          have_hi = 0;
        }
      }
    ...
    case SYS_BIN2HEX: {
      ...
      int32_t *out_arr = tc_resolve_ref(vm, out_ref);
      if (!out_arr || bin_len < 0) { TC_PUSH(vm, 0); break; }
      int32_t bin_cap = tc_ref_maxlen(vm, bin_ref);   // byte[] -> bytes, else slots
      int32_t out_cap = tc_ref_maxlen(vm, out_ref);
      if (bin_len > bin_cap) bin_len = bin_cap;
      if (bin_len * 2 + 1 > out_cap) bin_len = (out_cap - 1) / 2;
      // bin input accepts char[] OR byte[]. tmp holds a copy for int32 arrays;
      // for a byte[] tc_ref_bytes points straight into the region.
      const bool out_is_bytes = tc_ref_is_bytes(out_ref);
      uint8_t *tmp = (uint8_t*)special_malloc(bin_len > 0 ? bin_len : 1);
      if (!tmp) { TC_PUSH(vm, 0); break; }
      int32_t bn = 0;
      uint8_t *bp = tc_ref_bytes(vm, bin_ref, bin_len, tmp, bin_len > 0 ? bin_len : 1, &bn);
      if (bin_len > 0 && !bp) { free(tmp); TC_PUSH(vm, 0); break; }
      static const char hex[] = "0123456789abcdef";
      int32_t w = 0;
      for (int i = 0; i < bin_len; i++) {
        uint8_t b = bp[i];
        if (out_is_bytes) {
          ((uint8_t*)out_arr)[w++] = (uint8_t)hex[b >> 4];
          ((uint8_t*)out_arr)[w++] = (uint8_t)hex[b & 0x0F];
        } else {
          out_arr[w++] = (int32_t)hex[b >> 4];
          out_arr[w++] = (int32_t)hex[b & 0x0F];
        }
      }
      if (w < out_cap) {                 // NUL-terminate when room
        if (out_is_bytes) ((uint8_t*)out_arr)[w] = 0;
        else              out_arr[w] = 0;
      }
      free(tmp);
      TC_PUSH(vm, w);
...
// On a layout-hash / legacy mismatch, do NOT delete the old persist
// data — rename it to "<name>.pvs.bak" (one generation kept) so a
// schema-change flash is RECOVERABLE instead of a total data-loss
// (Andreas' Bat3: 27 cfg + tariff/SOC tables wiped on a single new
// persist var). Full name-keyed migration (PVS3) is tracked for 1.7.x;
// this is the cheap interim safety net.
static void tc_persist_demote(TcVM *vm) {
#ifdef USE_UFILESYS
  if (!ufsp) return;
  char bak[sizeof(vm->persist_file) + 5];
  snprintf(bak, sizeof(bak), "%s.bak", vm->persist_file);
  ufsp->remove(bak);
  if (ufsp->rename(vm->persist_file, bak)) {
    AddLog(LOG_LEVEL_INFO, PSTR("TCC: persist %s -> %s (recoverable)"),
           vm->persist_file, bak);
  } else {
    ufsp->remove(vm->persist_file);          // fallback: at least clear it
    AddLog(LOG_LEVEL_INFO,
           PSTR("TCC: persist %s reset (backup rename failed)"),
           vm->persist_file);
  }
#endif
}
  ...
  while (pc < end) {
    ...
    if (op == OP_STORE_WATCH) {
      ...
      switch (op) {
        ...
        case OP_LOAD_GLOBAL_BYTE: case OP_STORE_GLOBAL_BYTE:
        case OP_LOAD_GLOBAL_I16:  case OP_STORE_GLOBAL_I16:
        ...
        case OP_LOAD_LOCAL_BYTE: case OP_STORE_LOCAL_BYTE:
        case OP_LOAD_HEAP_BYTE:  case OP_STORE_HEAP_BYTE:
        case OP_LOAD_REF_BYTE:   case OP_STORE_REF_BYTE:
        case OP_LOAD_LOCAL_I16:  case OP_STORE_LOCAL_I16:
        case OP_LOAD_HEAP_I16:   case OP_STORE_HEAP_I16:
        case OP_LOAD_REF_I16:    case OP_STORE_REF_I16:
        case OP_LOAD_LOCAL_U16:  case OP_LOAD_HEAP_U16:
        case OP_LOAD_REF_U16:
        case OP_SYSCALL:
          skip = 1; break;
        ...
        case OP_INC_LOCAL:
          skip = 2; break;
        case OP_LK_OP_ST: case OP_LL_OP_ST:
          skip = 4; break;
        case OP_LL_CMP_JZ:   skip = 5; break;
        case OP_LLK_OP2_ST:  skip = 6; break;
        case OP_LK32_OP_ST:  skip = 7; break;
        case OP_LK32_CMP_JZ: skip = 8; break;
...
static int tc_vm_load(TcVM *vm, const uint8_t *binary, uint16_t size) {
  ...
  // Header length: V6 is self-describing (header_size in B20-21, =40); V5=20, V4=18, V3=16, V2=14.
  uint16_t header_size;
  if (version >= 6) {
    if (size < 40) return TC_ERR_BAD_BINARY;                     // minimum V6 header
    header_size = (B(20) << 8) | B(21);                          // self-describing — const pool starts here
    if (header_size < 40 || header_size > size) return TC_ERR_BAD_BINARY;
    uint32_t total_size = ((uint32_t)B(22) << 24) | ((uint32_t)B(23) << 16) |
    ...
    if (!tc_warned_prev6) {
      tc_warned_prev6 = true;
      AddLog(LOG_LEVEL_INFO, PSTR("TC: pre-v6 bytecode (older IDE) — recompile with the current IDE to enable version/ABI checking"));
    }
  }
  ...
  if (!vm->stack) return TC_ERR_OUT_OF_MEMORY;
  ...
  if (!vm->globals) return TC_ERR_OUT_OF_MEMORY;
  ...
  if (!vm->constants) return TC_ERR_OUT_OF_MEMORY;
...
#endif
  if (!vm->const_data) return TC_ERR_OUT_OF_MEMORY;
  ...
  if (heap_decl_size > 0) {
    ...
    if (total_heap > 0) {
      ...
      vm->heap_handles = (TcHeapHandle *)calloc(TC_MAX_HEAP_HANDLES, sizeof(TcHeapHandle));
      if (!vm->heap_handles) { free(vm->heap_data); vm->heap_data = nullptr; return TC_ERR_OUT_OF_MEMORY; }
...
static int tc_vm_step(TcVM *vm) {
  ...
  switch (op) {
    ...
    case OP_CALL_INDIRECT:
      // Indirect call via function pointer. Same frame setup as OP_CALL,
      // but the target address is popped off the data stack (low 16 bits)
      // instead of read from the next 2 bytes of bytecode. Caller pushes
      // args first, then the fn-ptr value, then this opcode.
      a = TC_POP(vm);                    // function bytecode address (u16 in low bits)
      addr = (uint16_t)(a & 0xFFFF);
      if (vm->frame_count >= TC_MAX_FRAMES) return TC_ERR_FRAME_OVERFLOW;
      {
        TcFrame *frame = &vm->frames[vm->frame_count];
        frame->return_pc = vm->pc;
    ...
    // ── Heap arrays ───────────────────────
    // ── Packed byte arrays ────────────────────────────────────────────────
    // Element i is byte i of the region: same memory, read differently. The
    // bound is the capacity in BYTES, i.e. slots * 4. For the local and global
    // variants the element count is not known at runtime, so the check is
    // against the end of the storage — same as the int32 variants do
    // (LOAD_LOCAL_ARR checks TC_MAX_LOCALS).
    case OP_LOAD_LOCAL_BYTE: {
      uint8_t base = tc_read_u8(vm);
      a = TC_POP(vm);                                   // index in bytes
      if (!vm->frames[vm->fp].locals) return TC_ERR_FRAME_INVALID;
      const int32_t grenze = (int32_t)(TC_MAX_LOCALS - base) * 4;
      if (base >= TC_MAX_LOCALS || a < 0 || a >= grenze) {
        tc_log_bounds("local byte", a, grenze, vm->pc); return TC_ERR_BOUNDS;
      }
      TC_PUSH(vm, (int32_t)((uint8_t *)(vm->frames[vm->fp].locals + base))[a]);
      break;
    }
    case OP_STORE_LOCAL_BYTE: {
      uint8_t base = tc_read_u8(vm);
      int32_t val = TC_POP(vm);
      a = TC_POP(vm);
      if (!vm->frames[vm->fp].locals) return TC_ERR_FRAME_INVALID;
      ...
      ((uint8_t *)(vm->frames[vm->fp].locals + base))[a] = (uint8_t)(val & 0xFF);
      break;
    }
    case OP_LOAD_GLOBAL_BYTE: {
      addr = tc_read_u16(vm);
      a = TC_POP(vm);
      const int32_t grenze = (addr < vm->globals_size) ? (int32_t)(vm->globals_size - addr) * 4 : 0;
      if (a < 0 || a >= grenze) { tc_log_bounds("global byte", a, grenze, vm->pc); return TC_ERR_BOUNDS; }
      TC_PUSH(vm, (int32_t)((uint8_t *)(vm->globals + addr))[a]);
      break;
    }
    case OP_STORE_GLOBAL_BYTE: {
      addr = tc_read_u16(vm);
      int32_t val = TC_POP(vm);
      a = TC_POP(vm);
      const int32_t grenze = (addr < vm->globals_size) ? (int32_t)(vm->globals_size - addr) * 4 : 0;
      if (a < 0 || a >= grenze) { tc_log_bounds("global byte", a, grenze, vm->pc); return TC_ERR_BOUNDS; }
      TC_GLOBALS_LOCK(vm);
      ((uint8_t *)(vm->globals + addr))[a] = (uint8_t)(val & 0xFF);
      TC_GLOBALS_UNLOCK(vm);
      break;
    }
    case OP_LOAD_HEAP_BYTE: {
      uint8_t handle = tc_read_u8(vm);
      a = TC_POP(vm);
      if (handle >= TC_MAX_HEAP_HANDLES || !vm->heap_data || !vm->heap_handles ||
          !vm->heap_handles[handle].alive) { tc_log_bounds("heap byte", a, 0, vm->pc); return TC_ERR_BOUNDS; }
      const int32_t grenze = (int32_t)vm->heap_handles[handle].size * 4;
      if (a < 0 || a >= grenze) { tc_log_bounds("heap byte", a, grenze, vm->pc); return TC_ERR_BOUNDS; }
      TC_PUSH(vm, (int32_t)((uint8_t *)(vm->heap_data + vm->heap_handles[handle].offset))[a]);
      break;
    }
    case OP_STORE_HEAP_BYTE: {
      uint8_t handle = tc_read_u8(vm);
      int32_t val = TC_POP(vm);
      a = TC_POP(vm);
      if (handle >= TC_MAX_HEAP_HANDLES || !vm->heap_data || !vm->heap_handles ||
      ...
      const int32_t grenze = (int32_t)vm->heap_handles[handle].size * 4;
      if (a < 0 || a >= grenze) { tc_log_bounds("heap byte", a, grenze, vm->pc); return TC_ERR_BOUNDS; }
      ((uint8_t *)(vm->heap_data + vm->heap_handles[handle].offset))[a] = (uint8_t)(val & 0xFF);
      break;
    }
    // Byte array as a parameter: the local slot holds the flagged ref; it is
    // resolved exactly as OP_LOAD_REF_ARR does, only read byte-wise.
    case OP_LOAD_REF_BYTE: {
      uint8_t li = tc_read_u8(vm);
      a = TC_POP(vm);
      if (li >= TC_MAX_LOCALS || !vm->frames[vm->fp].locals) return TC_ERR_BOUNDS;
      int32_t ref = vm->frames[vm->fp].locals[li];
      int32_t *basis = tc_resolve_ref(vm, ref);
      // Capacity of the REGION, four bytes per slot — deliberately not derived
      // from the ref's own kind: a mistyped argument would otherwise size the
      // bound by the caller's element width instead of the storage.
      const int32_t grenze = tc_ref_maxlen_slots(vm, ref) * 4;
      if (!basis || a < 0 || a >= grenze) { tc_log_bounds("ref byte", a, grenze, vm->pc); return TC_ERR_BOUNDS; }
      TC_PUSH(vm, (int32_t)((uint8_t *)basis)[a]);
      break;
    }
    case OP_STORE_REF_BYTE: {
      uint8_t li = tc_read_u8(vm);
      int32_t val = TC_POP(vm);
      a = TC_POP(vm);
      if (li >= TC_MAX_LOCALS || !vm->frames[vm->fp].locals) return TC_ERR_BOUNDS;
      ...
      const int32_t grenze = tc_ref_maxlen_slots(vm, ref) * 4;
      if (!basis || a < 0 || a >= grenze) { tc_log_bounds("ref byte", a, grenze, vm->pc); return TC_ERR_BOUNDS; }
      ((uint8_t *)basis)[a] = (uint8_t)(val & 0xFF);
      break;
    }

    // ── Packed 16-bit arrays ──────────────────────────────────────────────
    // Element i is the i-th 16-bit word of the region, so the bound is the
    // capacity in WORDS, i.e. slots * 2. Signedness lives in the load: the
    // stores below write the same sixteen bits for int16 and uint16 alike.
    case OP_LOAD_LOCAL_I16: case OP_LOAD_LOCAL_U16: {
      uint8_t base = tc_read_u8(vm);
      a = TC_POP(vm);                                   // index in words
      if (!vm->frames[vm->fp].locals) return TC_ERR_FRAME_INVALID;
      const int32_t grenze = (base < TC_MAX_LOCALS) ? (int32_t)(TC_MAX_LOCALS - base) * 2 : 0;
      if (a < 0 || a >= grenze) { tc_log_bounds("local i16", a, grenze, vm->pc); return TC_ERR_BOUNDS; }
      void *basis = (void *)(vm->frames[vm->fp].locals + base);
      TC_PUSH(vm, (op == OP_LOAD_LOCAL_I16) ? (int32_t)((int16_t *)basis)[a]
                                            : (int32_t)((uint16_t *)basis)[a]);
      break;
    }
    case OP_STORE_LOCAL_I16: {
      uint8_t base = tc_read_u8(vm);
      int32_t val = TC_POP(vm);
      a = TC_POP(vm);
      if (!vm->frames[vm->fp].locals) return TC_ERR_FRAME_INVALID;
      const int32_t grenze = (base < TC_MAX_LOCALS) ? (int32_t)(TC_MAX_LOCALS - base) * 2 : 0;
      if (a < 0 || a >= grenze) { tc_log_bounds("local i16", a, grenze, vm->pc); return TC_ERR_BOUNDS; }
      ((uint16_t *)(vm->frames[vm->fp].locals + base))[a] = (uint16_t)(val & 0xFFFF);
      break;
    }
    case OP_LOAD_GLOBAL_I16: case OP_LOAD_GLOBAL_U16: {
      addr = tc_read_u16(vm);
      a = TC_POP(vm);
      const int32_t grenze = (addr < vm->globals_size) ? (int32_t)(vm->globals_size - addr) * 2 : 0;
      if (a < 0 || a >= grenze) { tc_log_bounds("global i16", a, grenze, vm->pc); return TC_ERR_BOUNDS; }
      void *basis = (void *)(vm->globals + addr);
      TC_PUSH(vm, (op == OP_LOAD_GLOBAL_I16) ? (int32_t)((int16_t *)basis)[a]
                                             : (int32_t)((uint16_t *)basis)[a]);
      break;
    }
    case OP_STORE_GLOBAL_I16: {
      addr = tc_read_u16(vm);
      int32_t val = TC_POP(vm);
      a = TC_POP(vm);
      const int32_t grenze = (addr < vm->globals_size) ? (int32_t)(vm->globals_size - addr) * 2 : 0;
      if (a < 0 || a >= grenze) { tc_log_bounds("global i16", a, grenze, vm->pc); return TC_ERR_BOUNDS; }
      TC_GLOBALS_LOCK(vm);
      ((uint16_t *)(vm->globals + addr))[a] = (uint16_t)(val & 0xFFFF);
      TC_GLOBALS_UNLOCK(vm);
      break;
    }
    case OP_LOAD_HEAP_I16: case OP_LOAD_HEAP_U16: {
      uint8_t handle = tc_read_u8(vm);
      a = TC_POP(vm);
      if (handle >= TC_MAX_HEAP_HANDLES || !vm->heap_data || !vm->heap_handles ||
          !vm->heap_handles[handle].alive) { tc_log_bounds("heap i16", a, 0, vm->pc); return TC_ERR_BOUNDS; }
      const int32_t grenze = (int32_t)vm->heap_handles[handle].size * 2;
      if (a < 0 || a >= grenze) { tc_log_bounds("heap i16", a, grenze, vm->pc); return TC_ERR_BOUNDS; }
      void *basis = (void *)(vm->heap_data + vm->heap_handles[handle].offset);
      TC_PUSH(vm, (op == OP_LOAD_HEAP_I16) ? (int32_t)((int16_t *)basis)[a]
                                           : (int32_t)((uint16_t *)basis)[a]);
      break;
    }
    case OP_STORE_HEAP_I16: {
      uint8_t handle = tc_read_u8(vm);
      int32_t val = TC_POP(vm);
      a = TC_POP(vm);
      if (handle >= TC_MAX_HEAP_HANDLES || !vm->heap_data || !vm->heap_handles ||
      ...
      const int32_t grenze = (int32_t)vm->heap_handles[handle].size * 2;
      if (a < 0 || a >= grenze) { tc_log_bounds("heap i16", a, grenze, vm->pc); return TC_ERR_BOUNDS; }
      ((uint16_t *)(vm->heap_data + vm->heap_handles[handle].offset))[a] = (uint16_t)(val & 0xFFFF);
      break;
    }
    case OP_LOAD_REF_I16: case OP_LOAD_REF_U16: {
      uint8_t li = tc_read_u8(vm);
      a = TC_POP(vm);
      if (li >= TC_MAX_LOCALS || !vm->frames[vm->fp].locals) return TC_ERR_BOUNDS;
      int32_t ref = vm->frames[vm->fp].locals[li];
      int32_t *basis = tc_resolve_ref(vm, ref);
      const int32_t grenze = tc_ref_maxlen_slots(vm, ref) * 2;
      if (!basis || a < 0 || a >= grenze) { tc_log_bounds("ref i16", a, grenze, vm->pc); return TC_ERR_BOUNDS; }
      TC_PUSH(vm, (op == OP_LOAD_REF_I16) ? (int32_t)((int16_t *)basis)[a]
                                          : (int32_t)((uint16_t *)basis)[a]);
      break;
    }
    case OP_STORE_REF_I16: {
      uint8_t li = tc_read_u8(vm);
      int32_t val = TC_POP(vm);
      a = TC_POP(vm);
      if (li >= TC_MAX_LOCALS || !vm->frames[vm->fp].locals) return TC_ERR_BOUNDS;
      ...
      const int32_t grenze = tc_ref_maxlen_slots(vm, ref) * 2;
      if (!basis || a < 0 || a >= grenze) { tc_log_bounds("ref i16", a, grenze, vm->pc); return TC_ERR_BOUNDS; }
      ((uint16_t *)basis)[a] = (uint16_t)(val & 0xFFFF);
      break;
    }
    ...
    case OP_LL_CMP_JZ: case OP_LK32_CMP_JZ: {
      uint8_t c1 = tc_read_u8(vm);
      int32_t rhs;
      if (op == OP_LK32_CMP_JZ) { rhs = tc_read_i32(vm); }
      else { uint8_t c2 = tc_read_u8(vm);
             if (c2 >= TC_MAX_LOCALS) return TC_ERR_BOUNDS;
             if (!vm->frames[vm->fp].locals) return TC_ERR_FRAME_INVALID;
             rhs = vm->frames[vm->fp].locals[c2]; }
      uint8_t cop = tc_read_u8(vm);
      addr = tc_read_u16(vm);
      if (c1 >= TC_MAX_LOCALS) return TC_ERR_BOUNDS;
      if (!vm->frames[vm->fp].locals) return TC_ERR_FRAME_INVALID;
      int cerr = TC_OK;
      int32_t truth = tc_binop_i(cop, vm->frames[vm->fp].locals[c1], rhs, &cerr);
      if (cerr != TC_OK) return cerr;
      if (truth == 0) vm->pc = vm->code_offset + addr;   // branch on FALSE, like JZ
      break;
    }
    case OP_LK32_OP_ST: {
      uint8_t w1 = tc_read_u8(vm);
      int32_t wk = tc_read_i32(vm);
      uint8_t wop = tc_read_u8(vm);
      uint8_t wdst = tc_read_u8(vm);
      if (w1 >= TC_MAX_LOCALS || wdst >= TC_MAX_LOCALS) return TC_ERR_BOUNDS;
      if (!vm->frames[vm->fp].locals) return TC_ERR_FRAME_INVALID;
      int werr = TC_OK;
      int32_t wr = tc_binop_i(wop, vm->frames[vm->fp].locals[w1], wk, &werr);
      if (werr != TC_OK) return werr;
      vm->frames[vm->fp].locals[wdst] = wr;
      break;
    }
    case OP_LK_OP_ST: case OP_LL_OP_ST: {
      uint8_t s1 = tc_read_u8(vm);
      int32_t rhs;
      if (op == OP_LK_OP_ST) { rhs = (int32_t)(int8_t)tc_read_u8(vm); }
      else                   { uint8_t s2 = tc_read_u8(vm);
                               if (s2 >= TC_MAX_LOCALS) return TC_ERR_BOUNDS;
                               if (!vm->frames[vm->fp].locals) return TC_ERR_FRAME_INVALID;
                               rhs = vm->frames[vm->fp].locals[s2]; }
      uint8_t bop = tc_read_u8(vm);
      uint8_t dst = tc_read_u8(vm);
      if (s1 >= TC_MAX_LOCALS || dst >= TC_MAX_LOCALS) return TC_ERR_BOUNDS;
      if (!vm->frames[vm->fp].locals) return TC_ERR_FRAME_INVALID;
      int berr = TC_OK;
      int32_t r = tc_binop_i(bop, vm->frames[vm->fp].locals[s1], rhs, &berr);
      if (berr != TC_OK) return berr;
      vm->frames[vm->fp].locals[dst] = r;
      break;
    }
    // ── Superinstruction: locals[dst] = locals[y] OP_OUT (locals[z] OP_IN k) ──
    //
    // ⚠️ INNER FIRST, then outer -- exactly the order of the unfused form. On a
    // division by zero the INNER error has to surface; starting outside would
    // report the wrong one for `y / (z - z)`.
    case OP_LLK_OP2_ST: {
      uint8_t y  = tc_read_u8(vm);
      uint8_t z  = tc_read_u8(vm);
      int32_t k  = (int32_t)(int8_t)tc_read_u8(vm);
      uint8_t oi = tc_read_u8(vm);
      uint8_t oo = tc_read_u8(vm);
      uint8_t dst = tc_read_u8(vm);
      if (y >= TC_MAX_LOCALS || z >= TC_MAX_LOCALS || dst >= TC_MAX_LOCALS) return TC_ERR_BOUNDS;
      if (!vm->frames[vm->fp].locals) return TC_ERR_FRAME_INVALID;
      int e2 = TC_OK;
      int32_t inner = tc_binop_i(oi, vm->frames[vm->fp].locals[z], k, &e2);
      if (e2 != TC_OK) return e2;
      int32_t outer = tc_binop_i(oo, vm->frames[vm->fp].locals[y], inner, &e2);
      if (e2 != TC_OK) return e2;
      vm->frames[vm->fp].locals[dst] = outer;
      break;
    }
...
static int tc_vm_run_slice_ex(TcVM *vm, uint32_t max_instr, uint32_t budget_ms,
  ...
  if (!_dispatch_init) {
    ...
    // Packed byte arrays (docs/BYTE_ARRAYS.md). ⚠️ THIS table is the dispatcher
    // the ESP32 actually uses — the switch further up is the second path. An
    // opcode that only exists there dies on the device as "Unknown opcode"
    // (exactly what happened on the C3, 2026-08-21).
    _dispatch[0xA8] = &&_op_load_local_byte;  _dispatch[0xA9] = &&_op_store_local_byte;
    _dispatch[0xAA] = &&_op_load_global_byte; _dispatch[0xAB] = &&_op_store_global_byte;
    _dispatch[0xAC] = &&_op_load_heap_byte;   _dispatch[0xAD] = &&_op_store_heap_byte;
    _dispatch[0xAE] = &&_op_load_ref_byte;    _dispatch[0xAF] = &&_op_store_ref_byte;
    _dispatch[0xA7] = &&_op_ref_off;
    // Superinstructions
    _dispatch[0xB0] = &&_op_inc_local;
    _dispatch[0xB1] = &&_op_lk_op_st;  _dispatch[0xB2] = &&_op_ll_op_st;
    _dispatch[0xB3] = &&_op_lk32_op_st;
    _dispatch[0xB4] = &&_op_ll_cmp_jz; _dispatch[0xB5] = &&_op_lk32_cmp_jz;
    // Packed 16-bit arrays (docs/INT16_ARRAYS.md) — same warning as the byte
    // block above: an opcode that exists only in the switch dies here as
    // "Unknown opcode".
    _dispatch[0xB6] = &&_op_load_local_i16;  _dispatch[0xB7] = &&_op_store_local_i16;
    _dispatch[0xB8] = &&_op_load_global_i16; _dispatch[0xB9] = &&_op_store_global_i16;
    _dispatch[0xBA] = &&_op_load_heap_i16;   _dispatch[0xBB] = &&_op_store_heap_i16;
    _dispatch[0xBC] = &&_op_load_ref_i16;    _dispatch[0xBD] = &&_op_store_ref_i16;
    _dispatch[0xBE] = &&_op_load_local_u16;  _dispatch[0xBF] = &&_op_load_global_u16;
    _dispatch[0xC0] = &&_op_load_heap_u16;   _dispatch[0xC1] = &&_op_load_ref_u16;
    _dispatch[0xC2] = &&_op_llk_op2_st;
    _dispatch_init = true;
  }
  ...
  _op_load_local_byte: {
    ...
    const int32_t grenze = (base < TC_MAX_LOCALS) ? (int32_t)(TC_MAX_LOCALS - base) * 4 : 0;
    if (_a < 0 || _a >= grenze) { tc_log_bounds("local byte", _a, grenze, _pc); _err = TC_ERR_BOUNDS; goto _vm_exit; }
    TC_IPUSH((int32_t)((uint8_t *)(vm->frames[vm->fp].locals + base))[_a]);
    NEXT();
  }
  _op_store_local_byte: {
    uint8_t base = _RD_U8();
    int32_t val = TC_IPOP();
    _a = TC_IPOP();
    if (!vm->frames[vm->fp].locals) { _err = TC_ERR_FRAME_INVALID; goto _vm_exit; }
    const int32_t grenze = (base < TC_MAX_LOCALS) ? (int32_t)(TC_MAX_LOCALS - base) * 4 : 0;
    if (_a < 0 || _a >= grenze) { tc_log_bounds("local byte", _a, grenze, _pc); _err = TC_ERR_BOUNDS; goto _vm_exit; }
    ((uint8_t *)(vm->frames[vm->fp].locals + base))[_a] = (uint8_t)(val & 0xFF);
    NEXT();
  }
  _op_load_global_byte: {
    uint16_t base = _RD_U16();
    _a = TC_IPOP();
    const int32_t grenze = (base < vm->globals_size) ? (int32_t)(vm->globals_size - base) * 4 : 0;
    if (_a < 0 || _a >= grenze) { tc_log_bounds("global byte", _a, grenze, _pc); _err = TC_ERR_BOUNDS; goto _vm_exit; }
    TC_IPUSH((int32_t)((uint8_t *)(vm->globals + base))[_a]);
    NEXT();
  }
  _op_store_global_byte: {
    uint16_t base = _RD_U16();
    int32_t val = TC_IPOP();
    _a = TC_IPOP();
    const int32_t grenze = (base < vm->globals_size) ? (int32_t)(vm->globals_size - base) * 4 : 0;
    if (_a < 0 || _a >= grenze) { tc_log_bounds("global byte", _a, grenze, _pc); _err = TC_ERR_BOUNDS; goto _vm_exit; }
    TC_GLOBALS_LOCK(vm);
    ((uint8_t *)(vm->globals + base))[_a] = (uint8_t)(val & 0xFF);
    TC_GLOBALS_UNLOCK(vm);
    NEXT();
  }
  _op_load_heap_byte: {
    ...
    _a = TC_IPOP();
    if (handle >= TC_MAX_HEAP_HANDLES || !vm->heap_data || !vm->heap_handles ||
        !vm->heap_handles[handle].alive) { tc_log_bounds("heap byte", _a, 0, _pc); _err = TC_ERR_BOUNDS; goto _vm_exit; }
    const int32_t grenze = (int32_t)vm->heap_handles[handle].size * 4;
    if (_a < 0 || _a >= grenze) { tc_log_bounds("heap byte", _a, grenze, _pc); _err = TC_ERR_BOUNDS; goto _vm_exit; }
    TC_IPUSH((int32_t)((uint8_t *)(vm->heap_data + vm->heap_handles[handle].offset))[_a]);
    NEXT();
  }
  _op_store_heap_byte: {
    uint8_t handle = _RD_U8();
    ...
    const int32_t grenze = (int32_t)vm->heap_handles[handle].size * 4;
    if (_a < 0 || _a >= grenze) { tc_log_bounds("heap byte", _a, grenze, _pc); _err = TC_ERR_BOUNDS; goto _vm_exit; }
    ((uint8_t *)(vm->heap_data + vm->heap_handles[handle].offset))[_a] = (uint8_t)(val & 0xFF);
    NEXT();
  }
  _op_load_ref_byte: {
    uint8_t li = _RD_U8();
    _a = TC_IPOP();
    if (li >= TC_MAX_LOCALS || !vm->frames[vm->fp].locals) { _err = TC_ERR_BOUNDS; goto _vm_exit; }
    int32_t ref = vm->frames[vm->fp].locals[li];
    int32_t *basis = tc_resolve_ref(vm, ref);
    const int32_t grenze = tc_ref_maxlen_slots(vm, ref) * 4;   // see the step-path twin
    if (!basis || _a < 0 || _a >= grenze) { tc_log_bounds("ref byte", _a, grenze, _pc); _err = TC_ERR_BOUNDS; goto _vm_exit; }
    TC_IPUSH((int32_t)((uint8_t *)basis)[_a]);
    NEXT();
  }
  _op_store_ref_byte: {
    uint8_t li = _RD_U8();
    ...
    const int32_t grenze = tc_ref_maxlen_slots(vm, ref) * 4;   // see the step-path twin
    if (!basis || _a < 0 || _a >= grenze) { tc_log_bounds("ref byte", _a, grenze, _pc); _err = TC_ERR_BOUNDS; goto _vm_exit; }
    ((uint8_t *)basis)[_a] = (uint8_t)(val & 0xFF);
    NEXT();
  }
  ...
  _op_load_local_u16: {
    ...
    if (!vm->frames[vm->fp].locals) { _err = TC_ERR_FRAME_INVALID; goto _vm_exit; }
    const int32_t grenze = (base < TC_MAX_LOCALS) ? (int32_t)(TC_MAX_LOCALS - base) * 2 : 0;
    if (_a < 0 || _a >= grenze) { tc_log_bounds("local i16", _a, grenze, _pc); _err = TC_ERR_BOUNDS; goto _vm_exit; }
    void *basis = (void *)(vm->frames[vm->fp].locals + base);
    TC_IPUSH(_op == OP_LOAD_LOCAL_I16 ? (int32_t)((int16_t *)basis)[_a]
                                      : (int32_t)((uint16_t *)basis)[_a]);
    NEXT();
  }
  _op_store_local_i16: {
    uint8_t base = _RD_U8();
    ...
    const int32_t grenze = (base < TC_MAX_LOCALS) ? (int32_t)(TC_MAX_LOCALS - base) * 2 : 0;
    if (_a < 0 || _a >= grenze) { tc_log_bounds("local i16", _a, grenze, _pc); _err = TC_ERR_BOUNDS; goto _vm_exit; }
    ((uint16_t *)(vm->frames[vm->fp].locals + base))[_a] = (uint16_t)(val & 0xFFFF);
    NEXT();
  }
  _op_load_global_i16:
  _op_load_global_u16: {
    uint16_t base = _RD_U16();
    _a = TC_IPOP();
    const int32_t grenze = (base < vm->globals_size) ? (int32_t)(vm->globals_size - base) * 2 : 0;
    if (_a < 0 || _a >= grenze) { tc_log_bounds("global i16", _a, grenze, _pc); _err = TC_ERR_BOUNDS; goto _vm_exit; }
    void *basis = (void *)(vm->globals + base);
    TC_IPUSH(_op == OP_LOAD_GLOBAL_I16 ? (int32_t)((int16_t *)basis)[_a]
                                       : (int32_t)((uint16_t *)basis)[_a]);
    NEXT();
  }
  _op_store_global_i16: {
    uint16_t base = _RD_U16();
    int32_t val = TC_IPOP();
    _a = TC_IPOP();
    const int32_t grenze = (base < vm->globals_size) ? (int32_t)(vm->globals_size - base) * 2 : 0;
    if (_a < 0 || _a >= grenze) { tc_log_bounds("global i16", _a, grenze, _pc); _err = TC_ERR_BOUNDS; goto _vm_exit; }
    TC_GLOBALS_LOCK(vm);
    ((uint16_t *)(vm->globals + base))[_a] = (uint16_t)(val & 0xFFFF);
    TC_GLOBALS_UNLOCK(vm);
    NEXT();
  }
  _op_load_heap_i16:
  _op_load_heap_u16: {
    ...
    _a = TC_IPOP();
    if (handle >= TC_MAX_HEAP_HANDLES || !vm->heap_data || !vm->heap_handles ||
        !vm->heap_handles[handle].alive) { tc_log_bounds("heap i16", _a, 0, _pc); _err = TC_ERR_BOUNDS; goto _vm_exit; }
    const int32_t grenze = (int32_t)vm->heap_handles[handle].size * 2;
    if (_a < 0 || _a >= grenze) { tc_log_bounds("heap i16", _a, grenze, _pc); _err = TC_ERR_BOUNDS; goto _vm_exit; }
    void *basis = (void *)(vm->heap_data + vm->heap_handles[handle].offset);
    TC_IPUSH(_op == OP_LOAD_HEAP_I16 ? (int32_t)((int16_t *)basis)[_a]
                                     : (int32_t)((uint16_t *)basis)[_a]);
    NEXT();
  }
  _op_store_heap_i16: {
    ...
    const int32_t grenze = (int32_t)vm->heap_handles[handle].size * 2;
    if (_a < 0 || _a >= grenze) { tc_log_bounds("heap i16", _a, grenze, _pc); _err = TC_ERR_BOUNDS; goto _vm_exit; }
    ((uint16_t *)(vm->heap_data + vm->heap_handles[handle].offset))[_a] = (uint16_t)(val & 0xFFFF);
    NEXT();
  }
  _op_load_ref_i16:
  _op_load_ref_u16: {
    ...
    int32_t *basis = tc_resolve_ref(vm, ref);
    const int32_t grenze = tc_ref_maxlen_slots(vm, ref) * 2;
    if (!basis || _a < 0 || _a >= grenze) { tc_log_bounds("ref i16", _a, grenze, _pc); _err = TC_ERR_BOUNDS; goto _vm_exit; }
    TC_IPUSH(_op == OP_LOAD_REF_I16 ? (int32_t)((int16_t *)basis)[_a]
                                    : (int32_t)((uint16_t *)basis)[_a]);
    NEXT();
  }
  _op_store_ref_i16: {
    ...
    const int32_t grenze = tc_ref_maxlen_slots(vm, ref) * 2;
    if (!basis || _a < 0 || _a >= grenze) { tc_log_bounds("ref i16", _a, grenze, _pc); _err = TC_ERR_BOUNDS; goto _vm_exit; }
    ((uint16_t *)basis)[_a] = (uint16_t)(val & 0xFFFF);
    NEXT();
  }
  ...
  // ── Superinstruction: Schleifenkopf, Sprung bei FALSCH (wie das JZ, das es ersetzt) ──
  _op_ll_cmp_jz:
  _op_lk32_cmp_jz: {
    uint8_t _c1 = _RD_U8();
    int32_t _crhs;
    if (_op == OP_LK32_CMP_JZ) { _crhs = _RD_I32(); }
    else {
      uint8_t _c2 = _RD_U8();
      if (_c2 >= TC_MAX_LOCALS) { _err = TC_ERR_BOUNDS; goto _vm_exit; }
      if (!vm->frames[vm->fp].locals) { _err = TC_ERR_FRAME_INVALID; goto _vm_exit; }
      _crhs = vm->frames[vm->fp].locals[_c2];
    }
    uint8_t _cop = _RD_U8();
    _addr = _RD_U16();
    if (_c1 >= TC_MAX_LOCALS) { _err = TC_ERR_BOUNDS; goto _vm_exit; }
    if (!vm->frames[vm->fp].locals) { _err = TC_ERR_FRAME_INVALID; goto _vm_exit; }
    int _cerr = TC_OK;
    int32_t _truth = tc_binop_i(_cop, vm->frames[vm->fp].locals[_c1], _crhs, &_cerr);
    if (_cerr != TC_OK) { _err = _cerr; goto _vm_exit; }
    if (_truth == 0) { _pc = _coff + _addr; }
    NEXT();
  }
  _op_lk32_op_st: {
    uint8_t _w1 = _RD_U8();
    int32_t _wk = _RD_I32();
    uint8_t _wop = _RD_U8();
    uint8_t _wdst = _RD_U8();
    if (_w1 >= TC_MAX_LOCALS || _wdst >= TC_MAX_LOCALS) { _err = TC_ERR_BOUNDS; goto _vm_exit; }
    if (!vm->frames[vm->fp].locals) { _err = TC_ERR_FRAME_INVALID; goto _vm_exit; }
    int _werr = TC_OK;
    int32_t _wr = tc_binop_i(_wop, vm->frames[vm->fp].locals[_w1], _wk, &_werr);
    if (_werr != TC_OK) { _err = _werr; goto _vm_exit; }
    vm->frames[vm->fp].locals[_wdst] = _wr;
    NEXT();
  }
  // ── Superinstructions: locals[dst] = locals[src] OP (constant | locals[b]) ──
  _op_lk_op_st:
  _op_ll_op_st: {
    // _op is still the opcode just fetched -- it decides whether the right-hand
    // side is a constant or a second local.
    uint8_t _s1 = _RD_U8();
    int32_t _rhs;
    if (_op == OP_LK_OP_ST) { _rhs = (int32_t)(int8_t)_RD_U8(); }
    else {
      uint8_t _s2 = _RD_U8();
      if (_s2 >= TC_MAX_LOCALS) { _err = TC_ERR_BOUNDS; goto _vm_exit; }
      if (!vm->frames[vm->fp].locals) { _err = TC_ERR_FRAME_INVALID; goto _vm_exit; }
      _rhs = vm->frames[vm->fp].locals[_s2];
    }
    uint8_t _bop = _RD_U8();
    uint8_t _dst = _RD_U8();
    if (_s1 >= TC_MAX_LOCALS || _dst >= TC_MAX_LOCALS) { _err = TC_ERR_BOUNDS; goto _vm_exit; }
    if (!vm->frames[vm->fp].locals) { _err = TC_ERR_FRAME_INVALID; goto _vm_exit; }
    int _berr = TC_OK;
    int32_t _r = tc_binop_i(_bop, vm->frames[vm->fp].locals[_s1], _rhs, &_berr);
    if (_berr != TC_OK) { _err = _berr; goto _vm_exit; }
    vm->frames[vm->fp].locals[_dst] = _r;
    NEXT();
  }
  // ── Superinstruction: locals[dst] = locals[y] OP_OUT (locals[z] OP_IN k) ──
  //
  // ⚠️ THIS is the interpreter that RUNS programs. `tc_vm_step()` further up has
  // a `switch` over the same opcodes, but that one only single-steps; the fast
  // path dispatches through `_dispatch[]` to these labels. A superinstruction
  // added ONLY to the switch compiles, links and boots -- and then the device
  // answers "Unknown opcode" the first time the loop reaches it. Measured on
  // .39 on 2026-09-03, after the switch alone had looked complete.
  //
  // ⚠️ INNER FIRST, then outer -- exactly the order of the unfused form. On a
  // division by zero the INNER error has to surface, not the outer one.
  _op_llk_op2_st: {
    uint8_t _y  = _RD_U8();
    uint8_t _z  = _RD_U8();
    int32_t _k  = (int32_t)(int8_t)_RD_U8();
    uint8_t _oi = _RD_U8();
    uint8_t _oo = _RD_U8();
    uint8_t _d2 = _RD_U8();
    if (_y >= TC_MAX_LOCALS || _z >= TC_MAX_LOCALS || _d2 >= TC_MAX_LOCALS) {
      _err = TC_ERR_BOUNDS; goto _vm_exit; }
    if (!vm->frames[vm->fp].locals) { _err = TC_ERR_FRAME_INVALID; goto _vm_exit; }
    int _e2 = TC_OK;
    int32_t _inner = tc_binop_i(_oi, vm->frames[vm->fp].locals[_z], _k, &_e2);
    if (_e2 != TC_OK) { _err = _e2; goto _vm_exit; }
    int32_t _outer = tc_binop_i(_oo, vm->frames[vm->fp].locals[_y], _inner, &_e2);
    if (_e2 != TC_OK) { _err = _e2; goto _vm_exit; }
    vm->frames[vm->fp].locals[_d2] = _outer;
    NEXT();
  }
  // ── Superinstruction: locals[idx] += delta, kein Stapeleffekt ──
...
static void tc_vm_task(void *param) {
  ...
  if (vm->halted && vm->error == TC_OK) {
    ...
    if (tl_idx >= 0) {
      // Two-phase boot: hold here until the autoexec loader has run EVERY slot's
      // main() to addCommand/halt (it then clears hold_taskloop on all slots),
      // so a running TaskLoop never starves a still-initialising slot of VM
      // time/heap — the heap-tight slot-3 starvation Andreas hit on .107.
      while (slot->hold_taskloop && !slot->task_stop) {
...
static void TinyCStopVM(TcSlot *s) {
  ...
  // Drop the /tc_ui buttons this slot registered with webPageLabel(). They are
  // re-emitted on every main-page render from Tinyc->page_label[], so a stopped
  // slot left its buttons on the page until the next reboot — they even survived
  // a reload, and clicking one landed on an empty slot (mi-hol, discussion #118).
  // page_slot[] already records the owner, so only this slot's entries go.
  if (Tinyc) {
    uint8_t sidx_pg = 0xFF;
    for (uint8_t i = 0; i < TC_MAX_VMS; i++) {
      if (Tinyc->slots[i] == s) { sidx_pg = i; break; }
    }
    if (sidx_pg != 0xFF) {
      for (uint8_t p = 0; p < Tinyc->page_count; p++) {
        if (Tinyc->page_slot[p] == sidx_pg) Tinyc->page_label[p][0] = '\0';
      }
      while (Tinyc->page_count > 0 && !Tinyc->page_label[Tinyc->page_count - 1][0]) {
        Tinyc->page_count--;
      }
    }
  }
...
static bool TinyCStartVM(TcSlot *s) {
  ...
  s->task_stop = false;
  s->task_running = false;
  s->main_done = false;     // cleared before the task spawns so the serial autoexec loader
                            //   waits for THIS run's main() (set true in tc_vm_task)
  ...
  // Restore the registered command prefix on (re)start — BEFORE the VM task
  // re-runs main(). The teardown above (TinyCStopVM) cleared the live
  // cmd_prefix; for a transient stop+restart (SD->Flash sync resume, or the
  // #76 auto-stop) the sticky cmd_prefix_saved is still set, so the slot keeps
  // matching its BAT*/commands immediately instead of falling to
  // {"Command":"Unknown"} until main() happens to re-reach addCommand() — which
  // on a heap-tight boot/resume it may never do. main() re-running addCommand()
  // simply rewrites the same value (idempotent). Cleared only on a deliberate
  // stop/reset/program-replace, so this never resurrects a stopped slot.
  if (s->cmd_prefix_saved[0] && !s->cmd_prefix[0]) {
